...

void JedecSerialNum( JEDEC *pJedec, char *b )
{
    unsigned int i = 0, *h, t = 0;
/* 
 * Read a 6-bit ascii serial number and convert it
 */
    h = pJedec->jedec_b_assembly_serialnum;
    t = *h;
    for ( i = 0; i < 5; i++ ) {
	b[5+i] = ( ( t >> ( 6 * i ) ) & 0x3f ) + 32;
    }
    h = pJedec->jedec_b_manuf_date_m;
    t = *h;
    b[4] = ( ( t >> 0 ) & 15 ) + '0';
    b[3] = ( ( t >> 4 ) & 15 ) + '0';
    h = pJedec->jedec_b_manuf_date_y;
    t = *h;
    b[2] = ( ( t >> 0 ) & 15 ) + '0';
    h = pJedec->jedec_b_manuf_location;
    t = *h;
    if ( ( t & 0x0f ) == 1 ) {
	b[0] = 'N';
	b[1] = 'I';
 ...
 */
    sprintf( fw_rev, "%c%d.%d-%d", v_variant, v_major, v_minor, v_sequence );
/*
 ...
 */
    for ( i = 0;  i < ( ( ReadTsunamiCSR( CSR_CSC ) >> 14 ) & 0x03 ) + 1;  i++ ) {
 ...
 */
    for ( i = 0; i < MAX_PROCESSOR_ID; i++ ) {
	if ( cpu_mask & ( 1 << i ) ) {
	    if ( ( status = build_cpu_fru( hw_root, fru_root, i ) ) != SUCCESS )
		return( status );
	}
    }
    if ( ( status = build_memory_fru( hw_root, fru_root ) ) != SUCCESS )
	return( status );
/* 
 ...
 */
	...
	for ( j = 0;  j <= 31;  j++ ) {
	    for ( k = 0;  k <= 7;  k++ ) {
	    	if ( ( pb = get_matching_pb( i, 0, j, k, 0 ) ) != ( struct pb * )0 ) {
		    if ( pb->type == TYPE_PCI ) {
/*
 * Do not build a FRU node for the ISA I/O controller on hose 0, bus 0, slot 5, or
 * the on-board SCSI controller on hose 0, bus 0, slot 6.
 */
			if ( pb->hose == 0 && pb->bus == 0 && ( pb->slot == 5 || pb->slot == 6 ) )
			    continue;
			else if ( ( status = build_pci_fru( hw_root, fru_root, pb ) ) != SUCCESS )
			    return( status );
		    }
		}
	    }
 ...
 */
    ...
    pHw_smb_id->node_id_smb_rsrvd_32_38 = 0xFF;
    pHw_smb_id->node_id_smb_rsrvd_56_63 = 0xFF;
/* 
 * Initialize the System Mother Board (SMB) system resource configuration subpacket 
 */
    pSmb_node->smb_res.smb_sub.subpack_length = sizeof( SUBPKT_SMB_FRU5 ) + sizeof( SMB_PLATFORM );
    pSmb_node->smb_res.smb_sub.subpack_class = CLASS_CLIPPER; /* erfix */
    pSmb_node->smb_res.smb_sub.subpack_type = SUBPACK_SMB;
    pSmb_node->smb_res.smb_sub.subpack_rev = 1;
/* 
 * Fill in data for platform-specific SMB registers 
 */
    pSmb_node->smb_res.register_count = 5;
    pSmb_node->smb_platform.tig_rev = ( intig( NULL, 0xE00006 ) >> 5 ) & 0x7;
    pSmb_node->smb_platform.cchip_misc = ( ReadTsunamiCSR( CSR_MISC ) >> 32 ) & 0xff;
    pSmb_node->smb_platform.dchip_rev = ReadTsunamiCSR( CSR_DREV ) & 0x0f;
    pSmb_node->smb_platform.pchip0_ctrl = ( ReadTsunamiCSR( PCHIP0_PCTL ) >> 24 ) & 0x03;
    pSmb_node->smb_platform.pchip1_ctrl = ( ReadTsunamiCSR( PCHIP1_PCTL ) >> 24 ) & 0x03;
/* 
 ...
 */
    ...
    pNodeid_smb->node_id_smb_rsrvd_32_38 = 0xFF;
 ...
 */
    ...
    pNodeid_cpu->node_id_cpu_rsrvd_32_39 = 0xFF;
 ...
 */
    pCpu_node->cpu_res.cpu_sub.subpack_length = sizeof( SUBPKT_CPU_FRU5 );
 ...
 */
    ...
    pNodeid_iop->node_id_iop_rsrvd_8_39 = 0xFFFFFFFF;
    pNodeid_iop->node_id_iop_rsrvd_56_63 = 0xFF;
/*
 * Initialize the System Bus Bridge resource configuration subpacket 
 */
    pIop_node->iop_busbridge.busbridge_sub.subpack_length = sizeof( SUBPKT_BUSBRIDGE_FRU5 ) + sizeof( IOP_PCHIP );
    pIop_node->iop_busbridge.busbridge_sub.subpack_class = CLASS_CLIPPER; /* erfix */
...
/*
 * Initialize the platform-specific system bus bridge configuration registers
 */                                                                                                              
    switch ( iop_id ) {
    	case 0:	    /* Pchip 0 */
	    pIop_node->iop_pchip.pchip_pctl = ReadTsunamiCSR( PCHIP0_PCTL );
	    pIop_node->iop_pchip.pchip_err_mask = ReadTsunamiCSR( PCHIP0_PERRMASK );
	    pIop_node->iop_pchip.pchip_wsba0 = ReadTsunamiCSR( PCHIP0_WSBA0 );
	    pIop_node->iop_pchip.pchip_wsba1 = ReadTsunamiCSR( PCHIP0_WSBA1 );
	    pIop_node->iop_pchip.pchip_wsba2 = ReadTsunamiCSR( PCHIP0_WSBA2 );
	    pIop_node->iop_pchip.pchip_wsba3 = ReadTsunamiCSR( PCHIP0_WSBA3 );
	    pIop_node->iop_pchip.pchip_wsm0 = ReadTsunamiCSR( PCHIP0_WSM0 );
	    pIop_node->iop_pchip.pchip_wsm1 = ReadTsunamiCSR( PCHIP0_WSM1 );
	    pIop_node->iop_pchip.pchip_wsm2 = ReadTsunamiCSR( PCHIP0_WSM2 );
	    pIop_node->iop_pchip.pchip_wsm3 = ReadTsunamiCSR( PCHIP0_WSM3 );
	    pIop_node->iop_pchip.pchip_tba0 = ReadTsunamiCSR( PCHIP0_TBA0 );
	    pIop_node->iop_pchip.pchip_tba1 = ReadTsunamiCSR( PCHIP0_TBA1 );
	    pIop_node->iop_pchip.pchip_tba2 = ReadTsunamiCSR( PCHIP0_TBA2 );
	    pIop_node->iop_pchip.pchip_tba3 = ReadTsunamiCSR( PCHIP0_TBA3 );
	    break;
	case 1:	    /* Pchip 1 */
	    pIop_node->iop_pchip.pchip_pctl = ReadTsunamiCSR( PCHIP1_PCTL );
	    pIop_node->iop_pchip.pchip_err_mask = ReadTsunamiCSR( PCHIP1_PERRMASK );
	    pIop_node->iop_pchip.pchip_wsba0 = ReadTsunamiCSR( PCHIP1_WSBA0 );
	    pIop_node->iop_pchip.pchip_wsba1 = ReadTsunamiCSR( PCHIP1_WSBA1 );
	    pIop_node->iop_pchip.pchip_wsba2 = ReadTsunamiCSR( PCHIP1_WSBA2 );
	    pIop_node->iop_pchip.pchip_wsba3 = ReadTsunamiCSR( PCHIP1_WSBA3 );
	    pIop_node->iop_pchip.pchip_wsm0 = ReadTsunamiCSR( PCHIP1_WSM0 );
	    pIop_node->iop_pchip.pchip_wsm1 = ReadTsunamiCSR( PCHIP1_WSM1 );
	    pIop_node->iop_pchip.pchip_wsm2 = ReadTsunamiCSR( PCHIP1_WSM2 );
	    pIop_node->iop_pchip.pchip_wsm3 = ReadTsunamiCSR( PCHIP1_WSM3 );
	    pIop_node->iop_pchip.pchip_tba0 = ReadTsunamiCSR( PCHIP1_TBA0 );
	    pIop_node->iop_pchip.pchip_tba1 = ReadTsunamiCSR( PCHIP1_TBA1 );
	    pIop_node->iop_pchip.pchip_tba2 = ReadTsunamiCSR( PCHIP1_TBA2 );
	    pIop_node->iop_pchip.pchip_tba3 = ReadTsunamiCSR( PCHIP1_TBA3 );
	    break;
    	default:
	    printf( "build_iop_hw: Unknown IOP identifier = %d\n", iop_id );
	    return( 0 );
    }
/* 
 * Fill in the header extension information
 ...
 */        
    ...
    pNodeid_hose->node_id_hose_rsrvd_16_39 = 0xFFFFFF;
    pNodeid_hose->node_id_hose_rsrvd_56_63 = 0xFF;
/*
 * Build a hardware node for bus 0 on this hose
 */
 ...
 */
    ...
    pNodeid_bus->node_id_bus_rsrvd_24_39 = 0xFFFF;
 ...
 */
    ...
    pNodeid_ioctrl->node_id_io_ctrl_rsrvd_24_31 = 0xFF;
 ...
 */
    ...
    pNodeid_ioctrl->node_id_io_ctrl_hose = hose_id;
    pNodeid_ioctrl->node_id_io_ctrl_bus = bus_id;
    pNodeid_ioctrl->node_id_io_ctrl_ctrlr = slot_id;
    pNodeid_ioctrl->node_id_io_ctrl_rsrvd_24_31 = 0xFF;
    pNodeid_ioctrl->node_id_io_ctrl_rsrvd_56_63 = 0xFF;
/*
 ...
 */
    ...
    pNodeid_mem_sub->node_id_mem_sub_rsrvd_8_39 = 0xFFFFFFFF;
 ...
 */
    ...
    pNodeid_mem_ctrl->node_id_mem_ctrl_rsrvd_24_39 = 0xFF;
 ...
 */
	...
	if ( get_array_size( i ) != 0 ) {
	    ...
	    pSubpkt->memory_sub.subpack_class = CLASS_CLIPPER; /* erfix */
	    pSubpkt->memory_sub.subpack_type = SUBPACK_MEMORY;
	    pSubpkt->memory_sub.subpack_rev = 3;

	    pSubpkt->mem_id = i;
	    pSubpkt->register_count = 2;
	    pSubpkt->mmb_ctrl.cchip_csc = ReadTsunamiCSR( CSR_CSC );
	    switch ( i ) {
	    	case 0:
		    pSubpkt->mmb_ctrl.cchip_aar = ReadTsunamiCSR( CSR_AAR0 );
		    break;
		case 1:
		    pSubpkt->mmb_ctrl.cchip_aar = ReadTsunamiCSR( CSR_AAR1 );
		    break;
		case 2:
		    pSubpkt->mmb_ctrl.cchip_aar = ReadTsunamiCSR( CSR_AAR2 );
		    break;
		case 3:
		    pSubpkt->mmb_ctrl.cchip_aar = ReadTsunamiCSR( CSR_AAR3 );
		    break;
	    }
	    pSubpkt->mmb_ctrl.fru_count = 4;
	    pMemctrl_node->hd_memory_ctrl.size += sizeof( SUBPKT_MEMORY_FRU5 );
	    num_subpkts++;
	    pSubpkt = &pSubpkt->mmb_ctrl.fru + 1;
	}
    }
/* 
 * Fill in the header extension information
 ...
 */
    ...
    pNodeid_mem_desc->node_id_mem_desc_rsrvd_24_39 = 0xFF;
...
{
    int i, status = 0, orig_size;
    GCT_ROOT_NODE *pRoot;
    GCT_HANDLE node = 0;
    GCT_POWER_ENVIR_NODE *pPower_node;
    GCT_NODE_ID_POWER_ENVIR *pNodeid_power;
    unsigned __int64 tmp = 0;
    GCT_HD_EXT *pHd_ext;
    SUBPKT_SENSOR_FRU5 *pSensor;
    struct FILE *fp;
    ...
    unsigned char ps_present[ ] = { 7, 4 };
    unsigned char ps_ok[ ] = { 5, 6 };
 ...
 */
    ...
    pNodeid_power->node_id_power_rsrvd_16_31 = 0xFF;
/* 
 * Save original size before adding any subpackets.
 */
 ...
 * DC Power subpacket
 */
    if ( ( !( fail_register & ( 1 << ps_present[0] ) ) ) && 
	    ( !( func_register & ( 1 << ps_ok[0] ) ) ) ) {
	pSensor->sensor_sub.subpack_length = sizeof( SUBPKT_SENSOR_FRU5 );
	pSensor->sensor_sub.subpack_class = CLASS1;
	pSensor->sensor_sub.subpack_type = SUBPACK_SENSOR;
	pSensor->sensor_sub.subpack_rev = 2;
	pSensor->console_id = 0;
...
/*
 * System Fan subpackets
 */
    for ( i = 0;  i < 2;  i++ ) {
	if ( ( func_register & ( 1 << 1 ) ) || ( fail_register & ( 1 << fan_ok[i] ) ) ) {
	    pSensor->sensor_sub.subpack_length = sizeof( SUBPKT_SENSOR_FRU5 );
	    pSensor->sensor_sub.subpack_class = CLASS1;
	    pSensor->sensor_sub.subpack_type = SUBPACK_SENSOR;
	    pSensor->sensor_sub.subpack_rev = 2;
...
int build_fru_root( GCT_HANDLE hw_root ) 
{
    ...
    GCT_NODE_ID_FRU_ROOT *pFru_root_id;
    GCT_TLV *pTLV;
    unsigned char *pValue;
    struct DSRDB *dsrdb;
 ...
 */
    ...
    pFru_root_id->node_id_fru_root_rsrvd_08_15 = 0xFF;
    pFru_root_id->node_id_fru_root_rsrvd_16_31 = 0xFFFF;
    pFru_root_id->node_id_fru_root_rsrvd_32_63 = 0xFFFFFFFF; 
/* 
 * Save original size before adding to the descriptor
 */
    orig_size = pFru_root_node->hd_fru_root.size;
/* 
 ...
 */
    pTLV = &pFru_desc_node->fru_desc_info.fru_info.tlv_tag;
/*
 * Open the FRU EEPROM on the CPU daughter card and read 256
 * bytes of JEDEC-like data.
 */
    sprintf( device, "iic_cpu%d", cpu_id );
    if ( ( fp = fopen( device, "sr" ) ) == 0 ) {
 ...
 * Fill in the model number TLV field
 */
		build_tlv( pTLV, 1, "Fan", 3, pFru_desc_node );
		_NEXT_TLV( pTLV );
/*
 * Fill in the part number TLV field
 */
		switch ( pSensor->console_id ) {
		    case 0:
			build_tlv( pTLV, 1, "70-33599-01", 11, pFru_desc_node );
			break;
		    case 1:
			build_tlv( pTLV, 1, "70-33599-02", 11, pFru_desc_node );
			break;
		    default:
			build_tlv( pTLV, 1, " ", 1, pFru_desc_node );
			break;
...
int build_memory_fru( GCT_HANDLE hw_root, GCT_HANDLE fru_root ) 
{
    ...
    unsigned char dimm_pos_num[4][4] = {
	{ 0x00, 0x01, 0x02, 0x03 },
	{ 0x10, 0x11, 0x12, 0x13 },
	{ 0x20, 0x21, 0x22, 0x23 },
	{ 0x30, 0x31, 0x32, 0x33 }
    };
    unsigned char dimm_slot_num[4][4] = {
	{ 11, 14, 26, 28 },
	{  1,  6, 30, 32 },
	{ 13, 16, 25, 27 },
	{  5,  9, 29, 31 }
 ...
 */
    ...
    gct__insert_node( fru_root, node, GCT__INSERT_CHILD_LAST_SIB );
#if DEBUG
    printf( "Creating hose %d, bus %d, slot %d, function %d PCI FRU child %x for HW node %x\n", 
	    pb->hose, pb->bus, pb->slot, pb->function, node, hwnode );
#endif
/* 
 * Make a pointer to the FRU descriptor from the handle and initialize it.
 */
 ...
 * Fill in the part number TLV field
 */
		build_tlv( pTLV, 1, "30-48043-01", 11, pFru_desc_node );
		_NEXT_TLV( pTLV );
/*
 * Fill in the serial number TLV field
 */
 ...
 */
    ...
    pPartition->csl_res.csl_sub.subpack_length = sizeof( SUBPKT_CSL_FRU5 );
