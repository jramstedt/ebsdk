...
#if !MODE64
	unsigned int 	*read_data_lower, *read_data_upper;
	unsigned int	rd_1, rd_2;
...
*/
	    ...
	    /* See explanation why we need restriction on modulo for indices
	       greater than 21 below (before random call) */
/* THIS IS NOT NEEDED WHEN WE GET A 64 BIT COMPILER */
#if !MODE64
	    if (addr_rand_index > 21) restrict_modulo = 51671;
#endif
...
/* This modulo function will not be required once a 64 bit compiler is
   ...
   errors */
		     ...
		     addr_rand_index)) < (eptr->block_size>>2)) 
		{
	            ...
	            rand_addr = ((unsigned int)eptr->start_addr + 
		                (addr_seed<<2)); 
...
/* This modulo function will not be required once a 64 bit compiler is
   ...
   errors */
#if !MODE64
	            data_seed = random((data_seed % 18914), data_rand_index);
#else
			...
			/* Make sure data written and returned are the same */
				...
				no_sign_ext_addr);
	                ...
	                rand_addr = (unsigned int)rand_addr&0xfffffff8; 
		        no_sign_ext_addr = rand_addr;

#if !MODE64
		        /* Assign some addresses to the pointers */
		        read_data_upper = &rd_1;
		        read_data_lower = &rd_2;

			/* Will form a quadword of (data_seed | (~data_seed <<
			   32)) and write it to rand_addr and read it back in
			   read_Data_upper and read_data_lower */
		        do_stq_ldq(no_sign_ext_addr,data_seed,read_data_upper,
		                   read_data_lower);

		        if (*read_data_lower != (unsigned int)data_seed)
		        {    

		            /* error discovered */
		            load_error_info(8,data_seed,*read_data_lower,
				no_sign_ext_addr);
		        ...
		        if (*read_data_upper != (unsigned int)(~data_seed))
		        {    

		            /* error discovered */
		            load_error_info(9,(~data_seed),*read_data_upper,
				no_sign_ext_addr);
