...
int buildfru (int argc, char *argv[]) {
    ...
    for (i=0; argv[2][i]=toupper(argv[2][i]); i++);	/* Ensure its uppercase*/
    for (i=0; argv[3][i]=toupper(argv[3][i]); i++);	/* Ensure its uppercase*/
    for (i=0; argv[4][i]=toupper(argv[4][i]); i++);	/* Ensure its uppercase*/
    for (i=0; argv[5][i]=toupper(argv[5][i]); i++);	/* Ensure its uppercase*/
	...
	if (pNext->type == 0x15) {
		...
		while ( pFru_names[m].fru_id != 0) {
			...
			if ( ( pFrudesc->fru_desc_info.fru_diag_flag & DIAG_FLAG_TDD_ERR) &&  ( exp == rcv) ) {
				...
				hd_helper(getpcb()->pcb$a_stdout, 0, hd_buffer, 16, 1);
			    }
			} /* end TDD error */
			...
			if ( (pFrudesc->fru_desc_info.fru_diag_flag & DIAG_FLAG_SDD_ERR) && ( exp == rcv) ) {
				...
				hd_helper(getpcb()->pcb$a_stdout, 0, hd_buffer, 88, 1);
			    }
			} /* end SDD error */
			...
			if (pFrudesc->fru_desc_info.fru_diag_flag & DIAG_FLAG_CHKSUM_0_62) {
			    ...
			    hd_helper(getpcb()->pcb$a_stdout, 0, hd_buffer, 64, 1);
			}
...
unsigned __int64 find_fru_id(char *string, FRU_NAME_LIST list[]) { 
 ...
 *
-*/
void find_fru_name(unsigned __int64 nodeid, FRU_NAME_LIST list[], char *fruname) { 
...
unsigned int support_iic_fru( char *string, FRU_NAME_LIST list[]) { 
