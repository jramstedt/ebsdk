...
int driver_setmode (int mode, int log) {
	struct PCB	*pcb;
	int	pri;
	int	aff;
...
void driver_setmode_sub (int mode, int log, struct setmode_info *si) {
	int	i;
	struct pb	*pb;
	struct DDB	*ddb;
	struct DDB	**ddblist;
...
int fread( char *buffer, int size, int number, struct FILE *fp ) {
	...
	sysfault (100);
    (*(INT*) &fp->iob.io_count)++;
    bytes_read = (*fp->ip->dva->read)( fp, size, number, buffer );
    *(INT*) &fp->iob.bytes_read += bytes_read;
