...
/* Build this cpu's hwrpb slot */
    ...
    *(INT *)v = pal_virt_to_phys( id, PAL$IMPURE_BASE
...
/* new algorithm */
...
#endif
	{
        ...
        slot = (int)hwrpb_ptr + *hwrpb_ptr->SLOT_OFFSET + *hwrpb_ptr->SLOT_SIZE * id;
#endif
	bip = slot->STATE.SLOT$V_BIP;
	rc = slot->STATE.SLOT$V_RC;
	cv = slot->STATE.SLOT$V_CV;
	hr = slot->STATE.SLOT$V_HALT_REQUESTED;
 ...
 */
	if( hr == HALT_REQUEST$K_REMAIN_HALTED )
	    ...
	    {
		int destination = slot->STATE.SLOT$L_RSVD_MBZ2 & 0xff;
#if MODULAR
