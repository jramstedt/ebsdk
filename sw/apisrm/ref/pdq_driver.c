...
/* A linked list of all Eisa/fddi Allocinodes created....  
/* From this list, pdq_setmode() will change their running state to 
/* either DDB$STOP or DDB$START
*/	
struct PBQ _align (QUADWORD) pdqip;      /* See KERNEL_DEF.H */
...
-*/
void pdq_setmode (int mode) {
    struct PBQ *ipe;		 /* See KERNEL_DEF.H */

#if PDQ_TRACE
    pprintf("\nEntering pdq_setmode\n");
#endif

    /* A linked list of all FDDI Allocinodes created....  
    /* From this list we'll change their running state to arg==mode.
    */	
    spinlock(&spl_kernel);
    ipe = pdqip.flink;		/* head of linked list */
    spinunlock(&spl_kernel);
     
    /* Walk down the FDDI allocinode list, ie FDDI Port Drivers, and
    /* change their running mode to either DDB$STOP or DDB$START.
    */
    while (ipe != &(pdqip))
        {
    	if (mode == DDB$K_STOP) 
	    pdq_stop_driver((struct INODE *)ipe->pb); 	/* pb really point to INODES */

    	if (mode == DDB$K_START) 
    	    pdq_restart_driver((struct INODE *)ipe->pb);	    	    	
...
int pdq_start_PDQ (struct INODE *ip) {
    ...
    *(unsigned long*)(&(pdq->sa[4])) = pdq_read_csr(pdq, PDQ_HostData);
    ...
    sprintf(pdq->pb.alias, "__%c0", pdq->pb.controller + 'A');
    pdq->pb.alias[0] = toupper(pdq->pb.protocol[0]);
    pdq->pb.alias[1] = toupper(pdq->pb.protocol[1]);
...
int pdq_init_pb (struct INODE *ip, struct PDQ_PB *pdq) {
    ...
    */
    ...
    /*   Put this Inode on the DEFEA Inode queue.
    /*   This queue saves the Inode pointer of all fr port drivers started.
    /*   It will be used later to either shutdown/startup them via
    /*   pdq_setmode().
    */
    ipe     = (struct PBQ*)malloc(sizeof(struct PBQ));
    ipe->pb = (int *)ip;
    insq_lock(ipe, pdqip.blink);
...
*/
    ...
    /* Init the linked list of all DEFEA Allocinodes created....  
    /* From this list, pdq_setmode() will change their running state to 
    /* either DDB$STOP or DDB$START
    */
    pdqip.flink = &pdqip;
 ...
 *
-*/
pdq_init1_pb(struct PDQ_PB *pdq) {
    struct INODE  *ip;
    ...
    /*
    */
    sprintf(name, "%s%c0", pdq->pb.protocol, pdq->pb.controller + 'a');
