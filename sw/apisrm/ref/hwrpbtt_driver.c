...
#define bbssi(x,y) _bbssi(x,y)
#define bbcci(x,y) _bbcci(x,y)
...
int hwrpbtt_rxready( struct hwrpbtt_pb *pb )
    ...
    if( !pb->rxip )
	if ((*pb->hwrpb->RXRDY & (1 << pb->id)) && (pb->slot->RXLEN != 0))
	    {
	    pb->rxbuf = pb->slot->RX;
	    pb->rxip = 1;
...
void hwrpbtt_txsend( struct hwrpbtt_pb *pb, char c )
	...
	*pb->txbuf++ = '>';
	pb->txip = 0;
	pb->slot->TXLEN = (int)pb->txbuf - (int)pb->slot->TX;
	mb();
	bbssi( pb->id, pb->hwrpb->TXRDY );
	mb();
