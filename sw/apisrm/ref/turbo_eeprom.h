/*
 * turbo_eeprom.h 	7-May-1997	J. Kirchoff
 */

/*
Facility:

	TURBO Console, Diagnostics and System
...
Creation Date:
	...
	Conform to Product Support Data Logging Standards & Guidelines
	Revision 1.2, October 1st, 1990

Revisions:

	jrk	 7-May-1997	add OS log area

	twp	26-Apr-1993	modified the number of diag_sdd logs from 5
 				to 3 so everything fits

	twp	21-Apr-1993	modified diag_sdd structure
 
	twp	16-Dec-1992	modified diag_tdd structure for new timestamps
 
	jrk	 3-Aug-1992	remove console, repair, ver 5

	jrk	31-Jul-1992	chg lars size, ver 4

	jrk	 8-Jul-1992	make diag sdd just a block, correct definitions

	twp	29-Jun-1992	modified diag tdd and diag sdd logging

	29-Jun-1992	chg config entry
                                         
	24-Jun-1992	version 2, make long evs, add diag ttd, diag sdd
			and make operating system sdd right size.
*/
...
#define SCRIPT_POWERUP 0
#define SCRIPT_TEST 1
#define SCRIPT_BOOT 2

/*
 * Turbo EEPROM Area Definitions
 * Define as 1 to include area in EEPROM
 */

#define EEPROM_PRODUCT		1
#define EEPROM_CONSOLE		0
#define EEPROM_BOOT_SPEC	0
#define EEPROM_BOOT_PRIM	0
#define EEPROM_ENVIRON		1
#define EEPROM_CONS_PATCH	0
#define EEPROM_DIAG_PATCH	0
#define EEPROM_BOOT_SCRIPT	1
#define EEPROM_PWRUP_SCRIPT	1
#define EEPROM_TEST_SCRIPT	1
#define EEPROM_BOOT_ARGS	1
#define EEPROM_RMT_SUPPORT	0
#define EEPROM_FRU_DESCRIP	1
#define EEPROM_SERIAL		1
#define EEPROM_CONFIG		0
#define EEPROM_DIAG_LOG		0
#define EEPROM_DIAG_TDD		1
#define EEPROM_DIAG_SDD		1
#define EEPROM_SYM_LOG		1
#define EEPROM_HALT_COUNT	1
#define EEPROM_SERVICE_CALL	0
#define EEPROM_MOD_HIST		1
#define EEPROM_HALT_DATA	1
#define EEPROM_OS_LOG		1

/* EEPROM Hardware definitions */

#define TURBO_EEPROM_ADDRESS	0x97c00000	/* Base GBUS address for Turbo */
#define TURBO_EEPROM_BACKUP	0x90800000	/* Base GBUS address for Turbo */
#define GBUS_EEPROM		0xf3800000	/* Base Gbus address for Ruby */
#define EEPROM_WRITE_INTERVAL	(13)		/* 13 msec */
#define EEPROM_PAGESZ		4		/* 16 byte page - Xicor 2864 */
#define EEPROM_PAGEMSK		((1<<EEPROM_PAGESZ) - 1)
#define EEPROM_PAGEINC		(1<<EEPROM_PAGESZ)
...
#define eeprom_size_halt_data		((2*4)  + eeprom_s_watchreg + mchk$size + (9 * eeprom_s_nodereg) + (20 * eeprom_s_pcireg))
...
#define eeprom_s_halt_data	((3*4) + (1*4) + (2 * eeprom_s_halt_data_cpu))
...
struct HALT_LOG {
  ...
  int nodereg[eeprom_k_nbr_nodereg][9];
  int pcireg[eeprom_k_nbr_pcireg][20];
};
...
/*
 * definitions for stringlist routines
 * string lists are used to maintain environment string lists
 */
#define ENV_MAXSTRINGS	eeprom_k_nbr_evs	/* max number of environ strings */
#define ENV_STRINGBYTES	eeprom_size_evdata	/* max total length of environ strings */
...
struct environ_list {
	...
	char *strptrs[ENV_MAXSTRINGS];	/* vector of string pointers */
	char strbuf[ENV_STRINGBYTES];	/* strings themselves */
};
