...
{
int     pixel=0,byte=0,bank=0,chip=0;
 ...
 if (FailingByte<0)                                        
  ...
  for (i=0;i<4;i++)
        ...
        if (ebyte!=abyte)
                {
                byte=i;
                if ((address&7)>3)byte+=4;
                found = 1;
                break;
                }
        }
   }else {found=1;byte=FailingByte;}
   
 if (found)
        {
        printf ("\tVRAM Bank %d Pix %d Byte %d Exx--> %s \n",
                bank,
                chip8_table[byte].pixel,
                chip8_table[byte].byte,
                chip8_table[byte].text);
