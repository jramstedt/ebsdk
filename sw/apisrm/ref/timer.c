...
#if SABLE || MEDULLA || TURBO || MIKASA || AVANTI || ALCOR || K2 || MTU || CORTEX || YUKONA || TAKARA || APC_PLATFORM || REGATTA || MONET || EIGER
#define	MSEC_TICK	0
...
#if SABLE || MIKASA || AVANTI || MONET || K2 || MTU || TAKARA || APC_PLATFORM || REGATTA || WILDFIRE || MONET || EIGER
    if ((fp = fopen( "toy", "r+")) == NULL)                                               
	return;                                                                           
                                                                                          
#if MIKASA || AVANTI || MONET || K2 || MTU || TAKARA || APC_PLATFORM || REGATTA || WILDFIRE || MONET || EIGER
    data = 0x26;
#endif
#if SABLE     
#if GAMMA
...
*/
    data &= ~0x01;
#endif
#if SABLE || REGATTA || WILDFIRE || MONET || EIGER
    data |= 0x0e;
#endif
    fseek (fp, 0x0b, SEEK_SET);
    fwrite (&data, 1, 1, fp);
...
#if TURBO || RAWHIDE || PINNACLE || PC264 || REGATTA || WILDFIRE || MONET
...
#if PINNACLE || PC264 || REGATTA || WILDFIRE || MONET
	if (platform() == ISP_MODEL)
	    milliseconds = 1;
#endif
...
#define MULTIPLIER 100
	 ...
	 *  the CPU speed in nanoseconcds and MHz.
	 */
	freq2 = 1000000000;
