...
i2o_dump_exec_status( I2O_EXEC_STATUS_GET_REPLY *s ) {
...
i2o_dump_lct_notify( I2O_EXEC_LCT_NOTIFY_MESSAGE *ln ) {
 ...
 *
-*/
I2O_ExecLctNotify( struct i2o_pb *pb, int class_id, I2O_LCT *lct_head, int lct_size ) {
...
I2O_ExecIopReset( struct i2o_pb *pb ) {
    ...
    reset->MsgFlags = 0;
    reset->MessageSize = sizeof( *reset ) / 4;
	...
	pprintf("High %x %x Low %x %x\n", &reset->StatusWordHighAddress, reset->StatusWordHighAddress, 
				    &reset->StatusWordLowAddress, reset->StatusWordLowAddress );
	rst = ( *( volatile int *) &reset->StatusWordLowAddress ) & 0xF;
	pprintf("rst %x\n", rst );
	krn$_sleep( 1000 );
    }

    if ( rst != I2O_EXEC_IOP_RESET_IN_PROGRESS ) {
	pprintf("IOP reset failed\n");
	status = msg_failure;
    }
