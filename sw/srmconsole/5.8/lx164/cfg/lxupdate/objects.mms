! LXUPDATE Target-specific dependencies
i_objects = -
...
cp$obj:grp_base.obj : -
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	cp$src:alphamm.c+ansi+aputchar+atoi+blex+bshell+bshell_cmd+boot+builtins+call_backs+date+dynamic+entry+ev_action+exdep+externdata+filesys+hwrpb+ie+iobq_cmd+isacfg+kernel+kernel_support+kernel_alpha+memset+printf+probe_io+process+qscan+queue-
	+random+readline+regexp+semaphore+show_hwrpb+show_map+setshow+shellsort+startstop+strfunc+timer+uptime+xcmd+xlate+lx164+lx164_init+lx164_mcheck+lx164_io+memconfig_lx164+powerup_lx164+show_config_lx164+lfu_support_apc+lfu_startup_driver
	! Group "base" command length was ~594

!
! Group "driver" (56 files)
!
cp$obj:grp_driver.obj : -
	...
	cp$src:lfu_lx164_driver.c, $(lfu_lx164_driver_depend),-
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	+pci_nvram_driver+pdq_driver+dw_driver+dac960_driver+pks_driver+isp1020_driver+ew_driver+ei_driver+aic78xx_driver+aic78xx_him_hwm+aic78xx_him_rsm+aic78xx_him_xlm+lfu_lx164_driver+lfu_kzpsa_driver+lfu_defpa_defea_driver+iso9660_driver
	! Group "driver" command length was ~834
...
cp$obj:grp_command.obj : -
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	cp$src:lfu.c
	! Group "command" command length was ~134
...
cp$obj:encap_readme.obj : cp$src:sx164_readme
	mcr cp$kits:[encapsulate.new]encapsulate cp$src:sx164_readme cp$src:encap_readme.mar encap_readme -script
	$(macro64)/nolis/obj=cp$obj cp$src:encap_readme.mar

cp$obj:encap_save_nvram.obj : cp$src:save_nvram_lx164
	mcr cp$kits:[encapsulate.new]encapsulate cp$src:save_nvram_lx164 cp$src:encap_save_nvram.mar encap_save_nvram -script
	$(macro64)/nolis/obj=cp$obj cp$src:encap_save_nvram.mar

cp$obj:encap_restore_nvram.obj : cp$src:restore_nvram_lx164
	mcr cp$kits:[encapsulate.new]encapsulate cp$src:restore_nvram_lx164 cp$src:encap_restore_nvram.mar encap_restore_nvram -script
...
cp$obj:lfu_lx164_driver.obj : cp$src:lfu_lx164_driver.c, $(lfu_lx164_driver_depend)
