...
void i2o_bsa_dump_param_grp0( struct _I2O_BSA_DEVICE_INFO_SCALAR *bp ) {
...
void i2o_bsa_dump_transfer_msg( struct _I2O_BSA_READ_MESSAGE *bt ) {
 ...
 *
-*/
int i2o_bsa_transfer( struct FILE *fp, int len, unsigned char *c, int opcode )
    {
    int lbn;
    int t;
    struct INODE *inode;
 ...
 *
-*/
int i2o_bsa_send_read_write( struct ub *ub, unsigned char *c, int len, int lbn,
	int opcode ) 
    {
    int status;
    ...
    *(INT *) &bt->LogicalByteAddress.LowPart = lbn * BLOCK_SIZE;
    bt->FetchAhead = 0;
    bt->TimeMultiplier = 0;
    bt->ControlFlags = 0;
 ...
 *
-*/
void i2o_bsa_new_unit( struct i2o_pb *pb, int sb_index, struct _I2O_LCT_ENTRY *lct, int verbose ) {
    char *status;
    char name[32];
    int i, inode_status;
    struct sb *sb;
 ...
 *
-*/
void i2o_bsa_shutdown( struct sb *sb ) {
