...
#elif NAUTILUS || TINOSA || GLXYTRAIN
		{ "flash",  FLASH_ROM_START,	 1, FLASH_ROM_SIZE, ATTR$M_READ },
		{ "eerom",  EEROM_SECTOR_START,  1, EEROM_SIZE,     ATTR$M_READ | ATTR$M_WRITE },
		{ "srmrom", SRMROM_SECTOR_START, 1, SRMROM_SIZE,    ATTR$M_READ },
#elif SHARK
...
int from_write( struct FILE *fp, const int size, const int number, char *buf )
{
	...
	*fp->offset += nbytes;
	...
	while(nbytes > 0)
	{
		sector_size = flash_sector_size(fromp->base + *fp->offset);

		if (*fp->offset <= fromp->length)
		{
			length = min(sector_size, nbytes);
