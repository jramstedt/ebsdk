...
stubs1	= -
	...
	cp$obj:stub_machine_handle_vga.obj,-
...
stubs3	= -
	...
	cp$obj:stub_show_bios.obj,-
	cp$obj:stub_bios_support.obj,-
	cp$obj:stub_make_second_ide_channel.obj,-
	cp$obj:stub_run_bios_init.obj,-
	cp$obj:stub_bios_target_valid.obj,-
	cp$obj:stub_show_error_log.obj,-
	cp$obj:stub_clear_error_log.obj
...
cp$obj:stub_machine_handle_vga.obj	: cp$src:stub_machine_handle_vga.c
...
cp$obj:stub_show_bios.obj		: cp$src:stub_show_bios.c
cp$obj:stub_bios_support.obj		: cp$src:stub_bios_support.c
cp$obj:stub_make_second_ide_channel.obj	: cp$src:stub_make_second_ide_channel.c
cp$obj:stub_run_bios_init.obj		: cp$src:stub_run_bios_init.c
cp$obj:stub_bios_target_valid.obj	: cp$src:stub_bios_target_valid.c
cp$obj:stub_show_error_log.obj		: cp$src:stub_show_error_log.c
cp$obj:stub_clear_error_log.obj		: cp$src:stub_clear_error_log.c
...
!
cp$cfg:vlist.c : $(vlist_objects)
	mcr cp$kits:[scansym]scansym.exe cp$src:ref_symbols.txt $(cfg)objects.txt
	gawk -f cp$src:build_symtab.awk -f cp$cfg:sym_xxclusions.dat cp$src:ref_symbols.txt >cp$cfg:vlist.c
...
cp$obj:fptest_ieee.obj	: cp$src:fptest_ieee.c,-
			$(fptest_ieee_depend)
	$(cc_fp)/nomemb/assume=noalign/float=ieee_float-
	/mach/obj=cp$obj/opt=(inline=none,unroll=1)/lis=cp$lis cp$src:fptest_ieee.c

cp$obj:fptest_d.obj	: cp$src:fptest_d.c,-
			$(fptest_d_depend)
	$(cc_fp)/nomemb/assume=noalign/float=d_float-
	/mach/obj=cp$obj/opt=(inline=none,unroll=1)/lis=cp$lis cp$src:fptest_d.c

cp$obj:fptest_f.obj	: cp$src:fptest_f.c,-
			$(fptest_f_depend)
	$(cc_fp)/nomemb/assume=noalign/float=g_float-
	/mach/obj=cp$obj/opt=(inline=none,unroll=1)/lis=cp$lis cp$src:fptest_f.c

cp$obj:fptest_g.obj	: cp$src:fptest_g.c,-
			$(fptest_g_depend)
	$(cc_fp)/nomemb/assume=noalign/float=g_float-
	/mach/obj=cp$obj/opt=(inline=none,unroll=1)/lis=cp$lis cp$src:fptest_g.c
...
.sdl.mar
	$(sdl)/lang=macro=cp$src $(mms$source)
...
cp$obj:memtest_graycode.obj	: cp$src:memtest_graycode.mar,-
				  cp$src:cfw.mlb
	$(macro64) -
		cp$src:memtest_graycode.mar/lis=cp$lis/obj=cp$obj -
		+cp$src:cfw/lib
...
cp$exe:ev6_huf_decom.exe : $(ev6_huf_decom_objects)
	link/alpha/system=%x600000/nosysshr-
...
#cp$obj:x86_v3.olb : cp$src:x86_v3.olb
#	copy cp$src:x86_v3.olb cp$obj:x86_v3.olb/nolog/noconf
...
.IFDEF	AVMS_HOST	
	@ mms := mms
	@ mms/desc=cp$src:x86_v3.mms/macro="alpha=AVMS_HOST=1" cp$obj:x86_v3.olb
	@ dir/brief/noheader cp$obj:x86*.obj/out=cp$cfg:x86_objects.txt
	@ mcr cp$kits:[scansym]scansymbol.exe cp$src:x86_entry.dat cp$src:x86_data.dat cp$cfg:x86_objects.txt
	continue
.ELSE
	continue
.ENDIF
