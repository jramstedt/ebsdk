 ...
 * Modifications:
 *
 *	mar 	06-Jul-2000     Add two Digi DataFire Sync 2000 2P, 4P
 *	mar 	24-Mar-2000	Fix DEGPA-TA device id
 *	mar	14-Mar-2000	Add Ensoniq Sound card - This card, made by
 *                              Creative Labs Ltd., which owns Ensoniq, is
 *				etched with "Compaq Premier Sound". The card
 *                              does not have a Compaq subsystem vendor id.
 *	mar	01-Mar-2000	Rename KZPCC-CE to DPT PM3755 because it does not
 *				yet have a Compaq-specific vendor id.
 *	mar	01-Mar-2000	Add KZPCC-CE (dorado) - console recognition only
 *	mdr	21-Jan-2000	Add Intel 82559ER Ethernet Controller
 ...
 *	mar 	24-jan-2000	Add DEGPA-TA gigabit ethernet card             
 *	mar 	08-sep-1998	Add DEGPA-SA gigabit ethernet card             
...
int find_pb( char *device, int size, int (*callback)( ), protoargs int p1, int p2 )
    {
    int i;
    struct pb *pb;
...
int find_dev_pb( char *c )
	...
	{
	pb = pbs[i];
	sprintf( temp, "%s%s", pb->protocol, pb->controller_id );
	if( !strcmp_nocase( c, temp ) )
	    {
	    krn$_post( &pbs_lock );
	    return( pb );
	    }
...
struct io_device io_device[] = {
    ...
    { TYPE_PCI, 0x000f1011, 0xdef11011, "fw", "DEFPA", "fw", &fw_controllers },
    ...
    { TYPE_PCI, 0x00211000, 0x00000000, "n810", "SYM 53C1000", "pk", &pk_controllers },
    { TYPE_PCI, 0x00201000, 0x00000000, "n810", "SYM 53C1010", "pk", &pk_controllers },
    { TYPE_PCI, 0x00211000, 0x00000000, "n810", "SYM 53C1010-66", "pk", &pk_controllers },
    ...
    { TYPE_PCI, 0x12098086, 0x00000000, "ei", "Intel 82559ER Ethernet", "ei", &ei_controllers },
    ...
    { TYPE_PCI, 0x10121044, 0x00000000, 0, "DPT PM3755", 0, 0 },
    { TYPE_PCI, 0xB0600E11, 0x40700E11, "cpq", "CPQ SmartArray", "py", &py_controllers },
    { TYPE_PCI, 0x13711274, 0x00000000, 0, "Ensoniq Sound Card", 0, 0 },
    { TYPE_PCI, 0x0022114f, 0x00000000, 0, "DataFire SYNC 2000 2P", 0, 0 },
    { TYPE_PCI, 0x0021114f, 0x00000000, 0, "DataFire SYNC 2000 4P", 0, 0 },
#if 0                                                
    { TYPE_PCI, 0x09011000, 0x00000000, "usb", "USB", 0, &usb_controllers },
#endif                                               
    { TYPE_PCI, 0x06731095, 0x00000000, "usb", "CMD CSA-6730 USB", 0, &usb_controllers },
...
#endif
#if M1535C
    { TYPE_PCI, 0x153310b9, 0x00000000, "pci_isa", "Acer Labs M1535D", 0, &isa_eisa_count },
    { TYPE_PCI, 0x522910b9, 0x00000000, "dq", "Acer Labs M1535D IDE", "dq", &dq_controllers },
    { TYPE_PCI, 0x523710b9, 0x00000000, "usb", "Acer Labs M1535D USB", 0, &usb_controllers },
    { TYPE_PCI, 0x710110b9, 0x00000000, "acer_pmu", "Acer Labs M1535D PMU", 0, 0 },
    { TYPE_PCI, 0x545110b9, 0x00000000, "pci_audio", "Acer Labs M1535D Audio", 0, 0 },
    { TYPE_PCI, 0x545310b9, 0x00000000, "pci_modem", "Acer Labs M1535D Modem", 0, 0 },
#endif
    { TYPE_PCI, 0x002810de, 0x00000000, 0,			"ELSA GLoria Synergy II-32", 0, 0 },
    { TYPE_PCI, 0x70061022, 0x00000000, 0,			"AMD-751 Processor-PCI BR", 0, 0 },
    { TYPE_PCI, 0x70071022, 0x00000000, 0,			"AMD-751 AGP & PCI-PCI BR", 0, 0 },
    { TYPE_PCI, 0x700e1022, 0x00000000, 0,			"AMD-761 HOST-PCI BR",   0, 0 },
    { TYPE_PCI, 0x700f1022, 0x00000000, 0,			"AMD-761 AGP & PCI-PCI BR",    0, 0 },
    { TYPE_PCI, 0x8A135333, 0x00000000, "vga_bios", "S3 Trio 3D/2X", 0, 0 },
    { TYPE_PCI, 0x0525102B, 0x00000000, "vga_bios", "Matrox Millennium G400/2X", 0, 0 },	
    ...
    { TYPE_PCI, 0x030410ba, 0x4d361011, "vga_bios", "PowerStorm 350D", "vg", &vg_controllers },
    ...
    { TYPE_PCI, 0x8A225333, 0x00000000, "vga_bios", "S3 Savage 4", "vg", &vg_controllers },
    ...
    { TYPE_PCI, 0x00101069, 0x00101069, "dac960", "Mylex AcceleRAID 150", "dr", &dr_controllers },
    { TYPE_PCI, 0x10651011, 0x00201069, "dac960", "Mylex eXtremeRAID 1100", "dr", &dr_controllers },
    { TYPE_PCI, 0xba561069, 0x00401069, "dac960", "Mylex eXtremeRAID 2000", "dr", &dr_controllers },
    ...
    { TYPE_PCI, 0x000212ae, 0x600d1011, 0, "DEGPA-TA", 0, 0 },
    ...
    { TYPE_PCI, 0x81789004, 0x00000000, "aic78xx", "Adaptec AIC-7880", "pk", &pk_controllers },
    ...
    { TYPE_PCI, 0x00509005, 0x00000000, "aic78xx", "Adaptec AIC-7897", "pk", &pk_controllers },
    { TYPE_PCI, 0x00809005, 0x00000000, "aic78xx", "Adaptec 29160", "pk", &pk_controllers },
    { TYPE_PCI, 0x81789004, 0x00000000, "aic78xx", "Adaptec 2940UW", "pk", &pk_controllers },
    { TYPE_PCI, 0x00009004, 0x00000000, "aic78xx", "Adaptec AIC-78xx", "pk", &pk_controllers },
    { TYPE_PCI, 0x001f9005, 0x00000000, "aic78xx", "Adaptec AIC-7891", "pk", &pk_controllers },
    { TYPE_PCI, 0x00009005, 0x00000000, "aic78xx", "Adaptec AIC-78xx", "pk", &pk_controllers },
    ...
    { TYPE_PCI, 0xa0f70e11, 0x00000000, 0, "Yukon PCI Hot-Plug Controller", 0, 0 },
    { TYPE_PCI, 0x0001120F, 0x00000000, 0, "EssCom HiPPI", 0, 0 },
#endif
    { 0, 0, 0, 0, 0, 0 }
    };         
...
void probe_pci_bus( int hose, int min_slot, int max_slot )
    {
    char *name;
    char *protocol;
    int *controller;
	...
	for( function = 0; function <= max_function; function++ )
		...
		if (
		    (id == 0x000514d9) /*&&
			(api_debug_value(APIDB_ALLPCI) == 0)*/
		   )
			continue;			/* Tinosa fix.*/
#endif
		...
		if( BASE_CLASS( class ) == I2O_CLASS )
		    iod = &i2o_device;
		if( ( iod == 0 ) && ( class != 0 ) )
		    iod = find_io_device( pb->type, id, class );
		if( ( iod == 0 ) && ( sub_id != 0 ) )
		    iod = find_io_device( pb->type, id, sub_id );
		if( iod == 0 )
		    iod = find_io_device( pb->type, id, 0 );
		if( iod )
		    {
#if DRIVERSHUT
		    pb->tbl = iod;
			...
			sprintf( name, "%08X", id );
#if API_PLATFORM
		    if (BASE_CLASS( class ) == VGA_CLASS)
		    {
				pb->protocol = "vg";
				pb->device = "vga";
				pb->letter = &vg_controllers;
		    }
#endif
		    }
		pb->hardware_name = name;
 ...
 */
		if( ( iod ) && ( class == 0x0101 ) )
		    ...
		    else if( make_second_ide_channel( pb ) )
			{
			probe_number++;
			pb = malloc_noown( sizeof( *pb ) );
			pb->type = TYPE_PCI;
...
#if PROBE_EISA
void handle_pci_eisa_bridge( struct pb *pb )
    {
#if SABLE
    pci_bus = 1;
#endif
...
#if PROBE_ISA
void handle_pci_isa_bridge( struct pb *pb )
    {
#if DEBUG_PROBE
...
void handle_pci_bridge( struct pb *pb )
    {
...
void handle_pci_device( struct pb *pb )
    {
    int i;
    int j;
    int header;
	...
	if( i < num_bars )
		...
		if (id == MAKEVENID(PCI_VENDORID_AMD,PCI_DEVICEID_AMD751_PCI))
		{
			if (i == 0)
			{
				if (data == 0x8)										/* Then really means 0. Prefetch bit doesn't go away! */
					data  = 0;										/* Clear so no memory allocated. */
			}
			else
			if (i == 2)
			{
				data |= 0xff000000;										/* Force the tops bits of the requirements size to ff */
			}
		}
#endif
#if M1535C || M1543C
		/*
		*  Force the ACER IDE controllers (primary and secondary) to
		*  legacy addresses.
		*/
		...
		if (
		    (id == MAKEVENID(PCI_VENDORID_ACER,PCI_DEVICEID_ACER_USB)) ||
		    (id == MAKEVENID(PCI_VENDORID_ACER,PCI_DEVICEID_ACER_PMU))
		   )
		{
			data &= 0x0000ffff;
		}
	    ...
	    /*
	     * Compaq SmartArray requests a 32 bit I/O address but actually 
	     * needs a 16 bit I/O address.
	     */
...
void probe_propogate_pci_bus( struct pci_size_info *original_psi )
    {
    int i;
    int isa;
    int io_16_bit;
...
void probe_configure_pci_bus( struct pci_size_info *original_psi )
    {
    int i;
...
void pci_add_region( unsigned int base, unsigned int size, int type )
    {
#if DEBUG
...
int dec_pci_pci_bridge( struct pb *pb )
    {
    unsigned int vendor;
    unsigned int device;
...
void probe_eisa_bus( int hose, int bus )
    {
#if PROBE_EISA_SLOTS
    char *name;
    char *protocol;
...
#if PROBE_ISA
void probe_isa_bus( int hose, int bus )
{
#if PROBE_ISA_SLOTS
    unsigned int i;
    struct pb *pb;
...
void probe_assign_letters( int hose )
    {
    char *name;
    int i;
    int *letter;
	...
	if( pb->hose == hose )
		...
		*letter = *letter + 1;
		...
		if( pb->protocol )
		    ...
		    err_printf( " -- %s%s -- %s\n",
			    pb->protocol, pb->controller_id, pb->hardware_name );
		    }
#endif
		}
	    }
...
void add_driver_pointers( char *device, int setmode, int config, int establish )
    {
    struct io_device *iod;
...
void find_io_name( unsigned int id, unsigned int id_ext, char **name )
    {
...
void handle_cardbus_bridge( struct pb *pb ) 
    {
    struct pci_device_misc *pdm;
    struct pci_bridge_misc *pbm;
    struct pci_size_info *psi;
