...
void i2o_dump_reply ( struct _I2O_SINGLE_REPLY_MESSAGE_FRAME *rfa ) {
...
void i2o_dump_mfa ( struct _I2O_MESSAGE_FRAME *mfa ) {
...
void i2o_dump_mfhdr ( struct _I2O_CNSL_MFHDR *mfhdr ) {
...
void i2omsg_trace ( struct i2o_pb *pb, int op, int q, struct _I2O_MESSAGE_FRAME *mfa ) {
 ...
 *
-*/
void i2o_message_frame( struct _I2O_MESSAGE_FRAME *mf,  int sgl_offset, int flags,
		   int size, int target, int initiator, int function, 
		   struct _I2O_CNSL_MFHDR *mfhdr ) {
...
int I2O_MsgComplete( struct i2o_pb *pb, struct _I2O_CNSL_MFHDR *mfhdr ) {
 ...
 *
-*/
void I2O_ReplyFree( struct i2o_pb *pb, struct _I2O_MESSAGE_FRAME *mfa ) {
 ...
 *
-*/
void I2O_MsgSend( struct i2o_pb *pb, struct _I2O_CNSL_MFHDR *mfhdr ) {
    ...
    inbound = 0xFFFFFFFF;
    while (( inbound == 0xFFFFFFFF ) && ( inbound_num-- > 0 )) {
	krn$_sleep(100);
	inbound = ( void* ) I2O_read_mem( pb, INBOUND_FIFO );
	dprintf("FIFO read %x\n", p_args1(inbound) );
    }

    if ( inbound == 0xFFFFFFFF ) {
	pprintf("failed to obtain I2O request frame\n");
	return;
    }
	...
	case I2O_CNSL_CONTEXT_SEMAPHORE:
	    ...
	    while (( retry ) && ( !reply_received )) {
		post = krn$_wait( &mfhdr->rsp_t.sem );
		if (( post & 0xFF ) != I2O_REQUEST_SUCCESS ) {
		    retry--;
		    pprintf("\nwaiting for I2O reply - function %x pb %x mfhdr %x\n",
	    ...
	    stop_timeout( &mfhdr->rsp_t );
	    if ( !reply_received ) 
		pprintf("I2O request timeout - function %x pb %x mfhdr %x\n",
		    request->Function, pb, mfhdr );
	    break;
 ...
 *
-*/
void I2O_interrupt( struct i2o_pb *pb ) {
 ...
 *
-*/
void I2O_handle_reply( struct i2o_pb *pb ) {
 ...
 *
-*/
void I2O_write_mem( struct i2o_pb *pb, int offset, unsigned int data ) {
