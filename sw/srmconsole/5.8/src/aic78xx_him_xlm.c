...

/*$Header: /home/CVSROOT/sw/srmconsole/5.8/SRC/AIC78XX_HIM_XLM.C,v 1.2 2001/01/13 05:23:43 chris Exp $*/
/***************************************************************************
*                                                                          *
...
HIM_UINT8 SCSIVerifyAdapter (HIM_TASK_SET_HANDLE adapterTSH)
{
                        ...
                        &SCSI_ADPTSH(adapterTSH)->scsiHandle.ioHandle))
   {
      ...
      if (SCSI_ADPTSH(adapterTSH)->productID == hostID)
      {
         ...
         if (status == HIM_ADAPTER_NOT_SUPPORTED) 
         {
            return(status);
         }
         ...
         SCSIxUpdateDeviceTable(adapterTSH);
      ...
      switch(iob->function)
      {
         ...
         case HIM_RESET_BUS_OR_TARGET:
            ...
            else if (SCSI_TRGTSH(taskSetHandle)->typeTSCB == SCSI_TSCB_TYPE_ADAPTER)
            {
               ...
               /* determine if data bus hang condition and profile AP_SCSIOSMResetBusUseThrottledDownSpeedforDV is enabled */
...
#if SCSI_DOMAIN_VALIDATION
   profile->himu.TS_SCSI.AP_SCSIOSMResetBusUseThrottledDownSpeedforDV = (hhcb->SCSI_HF_dvThrottleOSMReset) ? HIM_TRUE : HIM_FALSE;
#endif   
...
/* May move into SCSI_GET_HW_INFORMATION */
   ...
   /* Phase delay profiles */  
   profile->AP_OEM1Specific.himu.TS_SCSI.AP_SCSICmndPhaseDelay = (HIM_UINT8)hwInformation.cmndPhaseDelay;
   profile->AP_OEM1Specific.himu.TS_SCSI.AP_SCSIDataPhaseDelay = (HIM_UINT8)hwInformation.dataPhaseDelay;
   profile->AP_OEM1Specific.himu.TS_SCSI.AP_SCSIStatusPhaseDelay = (HIM_UINT8)hwInformation.statusPhaseDelay;
                                     ...
                                     HIM_ADAPTER_PROFILE HIM_PTR profileMask)
{
   ...
   if (hwInformation.SCSI_PF_phaseDelaySupport)
   {
      profileMask->AP_OEM1Specific.himu.TS_SCSI.AP_SCSICmndPhaseDelay = 1;
      profileMask->AP_OEM1Specific.himu.TS_SCSI.AP_SCSIStatusPhaseDelay = 1; 
      profileMask->AP_OEM1Specific.himu.TS_SCSI.AP_SCSIDataPhaseDelay = 1;
   }
...
{                              
...
#if SCSI_DOMAIN_VALIDATION
   hhcb->SCSI_HF_dvThrottleOSMReset = profile->himu.TS_SCSI.AP_SCSIOSMResetBusUseThrottledDownSpeedforDV;
#endif
   ...
   if (hwInformation.SCSI_PF_phaseDelaySupport)
   {
      hwInformation.cmndPhaseDelay = (SCSI_UEXACT8)profile->AP_OEM1Specific.himu.TS_SCSI.AP_SCSICmndPhaseDelay;
      hwInformation.dataPhaseDelay = (SCSI_UEXACT8)profile->AP_OEM1Specific.himu.TS_SCSI.AP_SCSIDataPhaseDelay; 
      hwInformation.statusPhaseDelay = (SCSI_UEXACT8)profile->AP_OEM1Specific.himu.TS_SCSI.AP_SCSIStatusPhaseDelay;
   }
...
*                 
*********************************************************************/
void SCSIxAdjustTransferOption( 
         ...
         SCSI_HW_INFORMATION SCSI_LPTR phwInformation)
{
   ...
   {
      /*
       * Legacy hardware only support Single Transition transfer mode
       */   
      newDefaultProtocol = SCSI_ST_DATA;
   }
#else
   ...
   if ((newDefaultProtocol!=SCSI_ST_DATA) && (newDefaultProtocol!=SCSI_DT_DATA_WITH_CRC))
   {
   ...
   if (newDefaultWidth != currDefaultWidth)
   {
      ...
      else
      {
         ...
         /* xfer bit will be set when the new scsiOption get calculated. */
         ...
         if (phwInformation->targetInfo[scsiID].xferOption & SCSI_WIDEXFER)
         {
            newDefaultWidth = 16;
         }
      }
   ...
   if (deviceTable->SCSI_DF_syncDevice)
   {
      ...
      if ((newDefaultSpeed    != currDefaultSpeed)  ||
          ...
          (newDefaultProtocol != currDefaultProtocol))
      {
         scsiOptChanged = 1;  /* Indicate force negotiation is needed */
         ...
         else
         {
            ...
            /* 
             * At this point Async mode has been decided.
             * In order to enable SDTR, offset value need to be non-zero,
             * thus newdefaultOffset is set to currentOffset. The offset
             * will be correctly zeroed during SDTR (setForSync is 0).
             */
            newDefaultOffset = phwInformation->targetInfo[scsiID].SCSICurrentOffset;
            /*
...
void SCSIxQueueBusScan (SCSI_ADAPTER_TSCB HIM_PTR adapterTSH)
{
...
#if SCSI_NEGOTIATION_PER_IOB + SCSI_PARITY_PER_IOB
   if (probeFlag)
   {
   ...
   /* 'suppressNego' flag is HIM_TRUE.  With this change, it fixes a        */
   /* potential performance issue that might occur on our Neptune's NT and  */
   /* Netware driver.  For example, a targetTSH for TID 0/LUN 0 is already  */
   /* exist and currently running in wide and sync. mode.  Then comes a     */
   /* HIM_PROBE for TID 0/LUN 1 during run-time or outside the bus scan     */
   /* process.  In respond to this as in original CHIM code, CHIM will bring*/
   /* target back to async and narrow mode.  But it won't negotiate for     */
   /* wide/sync to the target or prepare the wide/sync negotiation to the   */
   /* target on the next IO.  The only way for CHIM to negotiate wide/sync  */
   /* to device is during Domain Validation process.  As the result, it will*/
   /* affect our driver performance.  To be consistence, the above change   */
   /* applies to our FMS U160/m driver also. */
   if (((targetTSCB->lunID == 0) && !probeFlag) || probeFlag)
   {
      ...
      else
      {
         ...
         /* Added OEM1 detection.  For OS like Netware, the driver will be */
         /* unloaded/loaded many times or the server will be brought down  */
         /* and up.  These steps invoke without any changes in scsi bus    */
         /* i.e. a sync. device will be running at sync. mode.  But, the   */
         /* driver will be running async. mode on the second or so load    */
         /* (because OEM1's BIOS boots in async and narrow mode) until the */
         /* Domain Validation completed.  So, force negotiation for narrow */
         /* and async. is needed here for OEM1 hardware.                   */
#if (SCSI_DOMAIN_VALIDATION || SCSI_OEM1_SUPPORT)
         if ((adapterTSH->domainValidationMethod && (targetTSCB->lunID == 0)) ||
             ((hhcb->OEMHardware == SCSI_OEMHARDWARE_OEM1) &&
              ((!probeFlag) || (probeFlag && (suppressNego == HIM_TRUE)))))
...
#endif /* (SCSI_DOMAIN_VALIDATION || SCSI_OEM1_SUPPORT) */
...
#if ((!SCSI_DOMAIN_VALIDATION) && SCSI_OEM1_SUPPORT)
         /* CHIM forces negotiation for Narrow and Sync. on current HIM_PROBE */
         /* if it is OEM1 hardware.  And because the Domain Validation is not */
         /* supported, CHIM need to force negotiation (either for Wide/Sync.) */
         /* on the next I/O.                                                  */
         else if (hhcb->OEMHardware == SCSI_OEMHARDWARE_OEM1)
         {
            /* restores negotiation if it's needed for the current target */
            SCSI_FORCE_NEGOTIATION(&targetTSCB->unitControl);
         }
...
#if SCSI_DOMAIN_VALIDATION
         if((transportSpecific = (HIM_TS_SCSI HIM_PTR)adapterTSH->iobProtocolAutoConfig->transportSpecific))
         {
...
#if (!SCSI_NEGOTIATION_PER_IOB)
         if (SCSI_DEVICE_TABLE(hhcb)[targetTSCB->scsiID].SCSI_DF_resetSCSI &&
...
#if ((!SCSI_DOMAIN_VALIDATION) && SCSI_OEM1_SUPPORT)
         /* CHIM forces negotiation for Narrow and Async. on current HIM_PROBE*/
         /* if it is OEM1 hardware.  And because the Domain Validation is not */
         /* supported, CHIM need to force negotiation (either for Wide/Sync.) */
         /* on the next I/O.                                                  */
         else if (hhcb->OEMHardware == SCSI_OEMHARDWARE_OEM1)
...
void SCSIxQueueDomainValidationEnhanced (SCSI_ADAPTER_TSCB HIM_PTR adapterTSH)
{
   ...
   {
      /* Build a Read Echo Buffer Descriptor */
      uexact8Pointer[0] = (SCSI_UEXACT8) 0x3C;
      uexact8Pointer[1] = (SCSI_UEXACT8) 0x03; /* Read Echo Buffer Descriptor */
      uexact8Pointer[8] = (SCSI_UEXACT8) SCSI_SIZE_REBD; /* Descriptor Length           */

      /* clear data buffer for read buffer descriptor */
      for (i = 0; i < SCSI_SIZE_REBD; i++)
      {
         ((SCSI_UEXACT8 HIM_PTR)adapterTSH->moreLocked)[SCSI_MORE_RWBDATA+i] = (SCSI_UEXACT8) ~i;
      }

      adapterTSH->SCSI_AF_dvState(targetTSCB->scsiID) = SCSI_DV_READBUFFER_DESCRIPTOR;
   }
   else if (adapterTSH->SCSI_AF_dvState(targetTSCB->scsiID) == SCSI_DV_READBUFFER_DESCRIPTOR)
#endif /* !SCSI_DISABLE_DV_ENHANCEMENTS */
   {
      /* setup write buffer cdb */
      uexact8Pointer[0] = (SCSI_UEXACT8) 0x3B;
      uexact8Pointer[1] = (SCSI_UEXACT8) 0x0A;          /* Echo Buffer        */
      uexact8Pointer[8] = (SCSI_UEXACT8) SCSI_SIZE_RWB; /* Echo Buffer Length */
   ...
   if (adapterTSH->SCSI_AF_dvState(targetTSCB->scsiID) == SCSI_DV_READBUFFER_DESCRIPTOR)
   {
      uexact32Value = 0x80000000 | SCSI_SIZE_REBD;
   }
   else
#endif /* !SCSI_DISABLE_DV_ENHANCEMENTS */
   {
...
HIM_UINT32 SCSIxPostDomainValidationBasic (HIM_IOB HIM_PTR iob)
{
   ...
   switch( iob->taskStatus)
   {
      ...
      case HIM_IOB_GOOD:
         ...
         for (i = 0; i < SCSI_SIZE_INQUIRY_DVBASIC - residual; i++)
         {
...
HIM_UINT32 SCSIxPostDomainValidationEnhanced (HIM_IOB HIM_PTR iob)
{
   ...
   {
      case HIM_IOB_GOOD:
         /* check what kind of command we issued last */
#if !SCSI_DISABLE_DV_ENHANCEMENTS
         if (adapterTSH->SCSI_AF_dvState(targetTSCB->scsiID) == SCSI_DV_READBUFFER_DESCRIPTOR)
         {
            break;  /* Continue on to the Write Buffer */
         }
         else 
#endif /* !SCSI_DISABLE_DV_ENHANCEMENTS */
                                     ...
                                     SCSI_TARGET_TSCB HIM_PTR targetTSCB)
{
   ...
   xferDE = (((scsiRate & SCSI_ENABLE_CRC) && !(scsiRate & SCSI_SINGLE_EDGE)) != 0) ? HIM_TRUE : HIM_FALSE;
   xferWide = ((scsiRate & SCSI_WIDE_XFER) != 0) ? HIM_TRUE : HIM_FALSE;
...
*
*********************************************************************/
void SCSIxUpdateDeviceTable (HIM_TASK_SET_HANDLE adapterTSH)
{
   SCSI_HHCB HIM_PTR hhcb = &SCSI_ADPTSH(adapterTSH)->hhcb;
   ...
   if (SCSIxBiosExist(adapterTSH) || SCSIxOptimaExist(adapterTSH))
   {
      ...
      for (i = 0; i < 16; i++)
      {
         ...
         else
         {
            ...
            /* from BIOS for bayScsiRate/Offset and bayDefaultRate/Offset. */
            /* Instead use setting in NVRAM or use defaults if no NVRAM    */
            /* because most of time the xfer options are narrow and async, */
            /* except for the case below.                                  */
            if (hhcb->OEMHardware != SCSI_OEMHARDWARE_OEM1)
#endif /* SCSI_OEM1_SUPPORT */
            {
               currentXferOption = (SCSI_UEXACT8) (xferOption[i] & SCSI_WIDEXFER);
...
*
*     Handle Adapter profile field AP_SCSIOSMResetBusUseThrottledDownSpeedforDV
*
*  Return Value:  void
*                  
*  Parameters:    hhcb
...
*                 and also checks if the profile AP_SCSIdvThrottleOSMReset  
*                 is enabled and if both true will throttle down the
...
void SCSIxHandledvThrottleOSMReset(SCSI_HHCB SCSI_HPTR hhcb)
{
   ...
   if ((hhcb->SCSI_HF_dvThrottleOSMReset) && (SCSIHDataBusHang(hhcb)))
   {
      scbNumber = OSD_INEXACT8(SCSI_AICREG(SCSI_hACTIVE_SCB(hhcb)));
                 
      				  
      if ((scbNumber != SCSI_NULL_SCB) && ((hiob = SCSI_ACTPTR[scbNumber]) != SCSI_NULL_HIOB))
			      
      {
         activeiob = SCSI_GETIOB(hiob);
           ...
           if (SCSI_TRGTSH(activeiob->taskSetHandle)->typeTSCB == SCSI_TSCB_TYPE_TARGET)
           {
              ...
              if ((adapterTSH->SCSI_AF_dvLevel(targetTSCB->scsiID) != SCSI_DV_DISABLE) && 
			     ((dvState == SCSI_DV_DONE) | (dvState == SCSI_DV_INQUIRY) |
			     (dvState == SCSI_DV_READBUFFER) | (dvState == SCSI_DV_WRITEBUFFER))	 
