...
int stop (int argc, char *argv[]) {
        int id, status;
	struct QSTRUCT qual[3];
	struct PCB *pcb;

	status = qscan (&argc, argv, "-", "drivers crash %dpartition", qual);
	...
	if (qual[0].present + qual[1].present + qual[2].present > 1) {
	    err_printf (msg_qual_conflict);
	    return msg_qual_conflict;
	}
	if (argc > 2 - qual[0].present - qual[2].present) {
	...
	if (qual[2].present) {
#if GALAXY && WILDFIRE
	    if (qual[2].value.integer < galaxy_partitions) {
		id = ((uint8 *)0x30)[qual[2].value.integer];
	    } else {
		err_printf (msg_einval);
		return msg_einval;
	    }
#else
	    err_printf (msg_qscan_noqual);
...
void start_drivers () {
	...
	if (drivers_running) {
	    driver_setmode (DDB$K_READY, 1, 0);
	} else {
	    for (i = 0; i < cb_ref; i++)
		fclose (cb_fp[i]);
	    cb_ref = 0;
	    driver_setmode (DDB$K_STOP, 1, 0);
	    reset_hardware (1);
	    krn$_set_console_mode (INTERRUPT_MODE);
	    driver_setmode (DDB$K_START, 1, 0);
	    drivers_running = 1;
	}
}
