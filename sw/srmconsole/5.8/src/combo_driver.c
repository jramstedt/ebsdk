...
#if RAWHIDE || (IPL_RUN > 19) || TURBO || WFSTDIO || MARVEL
...
#if SHARK
	if (api_debug_value(APIDB_UARTFIX))
	{
		/* BAD HORRIBLE CRAPPY FUCKING SHIT CRAP FIX THAT MUST BE GOT RID OF */
		if (combo_inportb (cp->port + LSR) & (LSR$M_OE|LSR$M_PE|LSR$M_FE|LSR$M_BI))
		{
			combo_outportb (cp->port + THR, 0);
		}
	}
#endif
