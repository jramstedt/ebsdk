 ...
 */
    ...
    pSmb_node->smb_res.smb_sub.subpack_class = CLASS_REGATTA;
 ...
 */
    ...
    pIop_node->iop_busbridge.busbridge_sub.subpack_class = CLASS_REGATTA;
 ...
 */
	...
	if ( get_array_size( i ) != 0 ) {
	    ...
	    pSubpkt->memory_sub.subpack_class = CLASS_REGATTA;
 ...
 */
			if ( pb->hose == 0 && pb->bus == 0 && ( pb->slot == 5 || pb->slot == 6 ) )
			    continue;
			else if ( ( status = build_pci_fru( hw_root, node, pb ) ) != SUCCESS )
			    return( status );
		    }
		}
	    }
