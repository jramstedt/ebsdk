...
struct csr_table pchip0_table[] = { 
    ...
    { "PLAT",  PCHIP0_PLAT  }, 
    { "PERR",  PCHIP0_PERROR },
    { "MASK",  PCHIP0_PERRMASK }, 
    { NULL, -1 } 
};

struct csr_table pchip1_table[] = { 
    { "WSBA0", PCHIP1_WSBA0 }, 
    { "WSBA1", PCHIP1_WSBA1 }, 
    { "WSBA2", PCHIP1_WSBA2 }, 
    { "WSBA3", PCHIP1_WSBA3 }, 
    ...
    { "PLAT",  PCHIP1_PLAT  }, 
    { "PERR",  PCHIP1_PERROR },
    { "MASK",  PCHIP1_PERRMASK }, 
    { NULL, -1 } 
};

char *dc21272_names[4] = {
    "CCHIP",
    "DCHIP",
    "PCHIP 0",
    "PCHIP 1"
...
struct csr_table *dc21272_tables[4] = { 
    cchip_table, 
    dchip_table, 
    pchip0_table, 
    pchip1_table 
...
void pc264_csr_dump( struct logout *frame )
{
    ...
    pprintf( "\nCPU %d\n", whoami( ) );
    pprintf( "DECchip 21272 CSRs:\n" );
    pprintf( "MISC:           %016X    DIR:            %016X\n",
		frame->system.cchip_misc, frame->system.cchip_dir );
    pprintf( "PERR0:          %016X    PERR1:          %016X\n",
		frame->system.pchip0_err, frame->system.pchip1_err );

    for ( j = 0;  j < 4;  j++ ) {
    	pprintf( "\n%s CSRs:\n", dc21272_names[j] );
	for ( t = dc21272_tables[j], i = 0;  t && t->name;  t++, i++ ) {
	    pprintf( "%-15.15s %016X%s", 
		     t->name,
		     ReadTsunamiCSR( t->offset ), i & 1 ? "\n" : "    " );
	}
	if ( i & 1 )
	    pprintf( "\n" );
    }
...
*/        
uint64 array_address_register[4];
void get_array_address_register(void)
{               
   array_address_register[0]= *(uint64 *)(0x801a0000100ul);
   array_address_register[1]= *(uint64 *)(0x801a0000140ul);
   array_address_register[2]= *(uint64 *)(0x801a0000180ul);
   array_address_register[3]= *(uint64 *)(0x801a00001c0ul);
}                          
                                               
void print_dimm_number(uint64 C_ADDR,int byte_number)
{               
  int dimm,i,j,bank=3,index[4]= {0,1,2,3};
