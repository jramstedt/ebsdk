...
/* build_fru_table used to be here.  Moved to after bootstrap loaded */
    ...
    *(INT *)slot->HALT_PC = 0x20000000 + image_start;
    ...
    for( i = 0; i < BOOT_L30PAGES; i++ )
	{
	((int *)l2pt)[64 + i*2] = 0x1101;
	((int *)l2pt)[65 + i*2] = l3pt0_pfn + i;
	}
...
int map_via_page_table( int *l3pt, uint64 pa, uint64 *va )
    ...
    pfn = virt_to_phys( pa ) / HWRPB$_PAGESIZE;
    for( i = 0; i < BOOT_L30PAGES * HWRPB$_PAGESIZE; i += 2 )
	{
	if( l3pt[i+1] == pfn )
	    {
