...
#if (TURBO || RAWHIDE || CLIPPER || WILDFIRE || MARVEL || PRIVATEER)
extern int mem_mask;
...
#if MARVEL
extern marvel_data;
extern marvel_bindings;
extern marvel_galaxy_config;
extern marvel_tree;
#endif
...
GCT_PLATFORM_TREE wildfire_tree[] = {
  {1, NODE_HW_ROOT,	0,	0,	0, {0}},
  {1, NODE_SW_ROOT,	0,	0,	0, {0}},
  {0}
}; 

GCT_GALAXY_NVRAM generic_galaxy_config[] = {
  {NODE_HARD_PARTITION,	0,	0,	NO_INIT, {0}},
  {NODE_COMMUNITY,	0,	0,	NO_INIT, {0}},
  {0}
};

GCT_PLATFORM_TREE wildfire_tree_template[] = {
  {1, NODE_HW_ROOT,	0,	0,	0, {0}},
  {0}
};

GCT_GALAXY_NVRAM wildfire_galaxy_config[] = {
  {NODE_HARD_PARTITION,	0,	0,	NO_INIT, {0}},
  {NODE_COMMUNITY,	0,	0,	NO_INIT, {0}},
  {0}
};

char wildfire_id[] = "GALAXY_ID_WILD01";
 ...
 *   system type cod
 */
GCT_PLATFORM_DATA wildfire_data[] = {
  TWO_MEG,
  64*ONE_MEG,
  64*ONE_MEG,
  8*ONE_MEG,
...
GCT_BINDINGS wildfire_bindings[] = {
...
/*+
 * ============================================================================
 * = gct - Build a Galaxy configuration tree.                                 =
 * ============================================================================
 *
 * OVERVIEW:
 *  
...
int gct() {
    ...
    pPlatform_bindings = &marvel_bindings;
...
#else
	hwrpb_ptr = hwrpb;
	if ( !hwrpb_ptr || !(hwrpb_ptr->FRU_OFFSET[0]) ) {
#if (TURBO || RAWHIDE)
...
/*  gct_init$populate_tree - Build the tree by moving xxxxx_tree into platform_tree  */
/*  Current Platforms using this path 
	Clipper
	PC264
	Wildfire
	Privateer
*/
	...
	if (tree_debug) pprintf("GCT Configuring SW/HW/FRU branches\n");
	gct_init$platform_tree();
...
int gct__partition_init( GCT_HANDLE node, GCT_HANDLE community,
 GCT_HANDLE partition, void *type_specific) {
  ...
  unsigned __int64 *hwrpb_ptr;

  hwrpb_ptr = (unsigned __int64 *) type_specific;
  pPartition = (GCT_PARTITION_NODE *) _GCT_MAKE_ADDRESS(node);
  pPartition->hwrpb   = hwrpb_ptr[0];
  pPartition->os_type = hwrpb_ptr[1];
 ...
 *			partition, community or hard partition.
 *
  ...
  */
  if ( pNode->owner ) {
    if ( pNode->owner == sw_node ) status = SUCCESS;
	 ...
	 status = BADOWNER;
    if (tree_debug) printf("Already Owned\n");
    gct__unlock_tree( UNLOCK_NO_UPDATE, GCT__INTERNAL_ID );
    return ( status );
  }

 /*
  *   See what the Nodes are...
  */
  pNode    = _GCT_MAKE_ADDRESS( sw_node );
  pHW_Node = _GCT_MAKE_ADDRESS( hw_node );

  if ( pNode->type == NODE_SW_ROOT ) {
    if (tree_debug) printf("Assign HW type %x (%x) to SW_Root (%x)\n",pHW_Node->type, hw_node, sw_node);
   /*
    *   Assign the hardware
    */
    status = gct__assign_hw_to_sw_root( sw_node, hw_node );
  }

  if (pNode->type == NODE_HARD_PARTITION) {
    if (tree_debug) printf("Assign HW type %x (%x) to Hard_Partition(%x)\n",pHW_Node->type, hw_node,sw_node);
   /*
    *   Assign the hardware
    */
    status = gct__assign_hw_to_hard_part( sw_node, hw_node );
  }

  if (pNode->type == NODE_COMMUNITY) {
    if (tree_debug) printf("Assign HW type %x (%x) to Community(%x)\n",pHW_Node->type, hw_node,sw_node);
   /*
    *   Cake.  Its a community.  Assign the hardware.
    */
    status = gct__assign_hw_to_community( sw_node, hw_node );
  }

  if (pNode->type == NODE_PARTITION) {
    if (tree_debug) printf("Assign HW type %x (%x) to Partition(%x)\n",pHW_Node->type, hw_node,sw_node);
   /*
    *   Assign the hardware
    */
    status = gct__assign_hw_to_partition( sw_node, hw_node );
  }
...
int gct__lock_tree( int lock_type, GCT_ID caller_id, int lock_caller, unsigned __int64 retry) {
  ...
  /* We are touching the Tree 
   * Update the Transient Level
   */
  if (lock_type == LOCK_FOR_UPDATE)
    pRoot->transient_level = pRoot->current_level + 1;
...
int gct__unlock_tree( int lock_type, GCT_ID caller_id) {
  ...
  /* We are done touching the Tree 
   * Update the Transient Level so they match 
   */
  if (lock_type == UNLOCK_UPDATE) {
    pRoot->current_level++;
  }
  pRoot->transient_level = pRoot->current_level;
 ...
 *	Abstract:
 *
 *	  This routine attempts to assign SW_Root ownership to a node and
 *	  it's children.
 *
 *	  Hardware components may be assigned a software owner.  The valid
 *	  ownership states are:
 *
 *	  SW_ROOT		Not owned
 *	  HARD_PARTITION	Owned by HP, Deallocation of shared resouce
 ...
 *	  In addition, if the SAVED_OWNER binding is NOT the SW_ROOT,
 *	  then all ancestors between the node, and the SAVED_OWNER
 *	  binding must *also* have the same partition or community
 *	  owner.  This also means that all descendents of the SAVED_OWNER
 *	  binding must have the same partition owner.
 *
 *	  If the SAVED_OWNER binding is to a Community, then it
...
int gct__assign_hw_to_sw_root( GCT_HANDLE sw_root, GCT_HANDLE hw_node ) {
  ...
  *
  *   Now, check the simplest case - a SAVED_OWNER
  *   binding to the HW_ROOT
  */
  if (pNode->saved_owner == sw_root) {
	/*
	*  Success.  The simple case that the
	*  config pointer is the SW root
	*/
	pNode->owner = sw_root;
	pNode->current_owner = sw_root;

	/*
	*  Assign all the children (owned)
	*/
 	tnode = pNode->child;
 	while (tnode) {
	    pTemp = _GCT_MAKE_ADDRESS( tnode);
	    pTemp->owner = sw_root;
	    pTemp->current_owner = sw_root;
 ...
 *	  When the hardware node is assigned an partition owner,
 *	  all of it's descendents must be owned by the partition.
 *	  In addition, if the SAVED_OWNER binding is NOT the HW_ROOT,
 *	  then all ancestors between the node, and the SAVED_OWNER
 *	  binding must *also* have the same partition or community
 *	  owner.  This also means that all descendents of the SAVED_OWNER
 *	  binding must have the same partition owner.
...
int gct__assign_hw_to_hard_part( GCT_HANDLE hardpartition, GCT_HANDLE hw_node ) {

  GCT_NODE *pHard_Partition, *pPartition, *pCommunity, *pNode, *pTemp, *pConfig;
  GCT_HANDLE sw_root, tnode, community;
  ...
  */
  if (pNode->saved_owner == sw_root) {
	...
	*/
	...
	pNode->current_owner = hardpartition;
	...
	*/
 	...
 	while (tnode) {
	    ...
	    pTemp->current_owner = hardpartition;
	    if (tree_debug) printf("assign %x owner %x current_owner %x\n",pTemp->type,pTemp->owner,pTemp->current_owner);
 	    tnode = gct__lookup_node( pNode->child, tnode, FIND_ANY, 0, 0, 0, 0, 0);
	}
	return (SUCCESS);
 ...
 *	  binding must have the same partition owner.
 *
 *	  If the CONFIG binding is to a Community, then it
 *	  must be the same community as the partition.
 *
 *	  Simple?
 *  
...
int gct__assign_hw_to_partition( GCT_HANDLE partition, GCT_HANDLE hw_node ) {

  GCT_NODE *pPartition, *pCommunity, *pHardPartition, *pNode, *pTemp, *pConfig;
  GCT_HANDLE hw_root, sw_root, tnode, community, hardpartition; 
  ...
  */
  if (pCommunity->type != NODE_COMMUNITY) {
	printf("Is NOT a Community\n");
	return (NOTCOMMUNITY);
  }

/*
  *  Get the hard partition node
  */
  hardpartition = pCommunity->owner;
  pHardPartition = _GCT_MAKE_ADDRESS(hardpartition);

 /*
  *   Is the node already owned by the specified partition?
  */
  if (pNode->owner == partition)  return (SUCCESS);
 ...
 /*
  *  One last things to check... we don't want to
  *  succeed if any of the children are
  *  owned (this can only happen if there is a bug
  *  in some callback code).
  *
  */
  ...
  *  OK.  We're fine.
  *
  *  The Saved_Owner binding is the community, then
  *  assign the node and its descendents to the partition.
  *
  */
  pConfig = _GCT_MAKE_ADDRESS(pNode->saved_owner);
...
int gct__assign_hw_to_community( GCT_HANDLE community, GCT_HANDLE hw_node, unsigned int active) {
 ...
 /*
  *   Now, check the next simplest case - a SAVED_OWNER
  ...
  *   node and the SAVED_OWNER are not bound to another community or partition
  */
  tnode = hw_node;
  while (tnode != pNode->config) {
    pTemp = _GCT_MAKE_ADDRESS( pNode->parent);
...
int gct__assign_hw_to_community( GCT_HANDLE community, GCT_HANDLE hw_node ) {

  GCT_NODE *pCommunity, *pNode, *pTemp, *pConfig;
  GCT_HANDLE sw_root, tnode;
 ...
 /*
  *   Now, check the next simplest case - a SAVED_OWNER
  *   binding to the SW_ROOT, with no owner
  */
  if (pNode->saved_owner == sw_root) {
...
void galaxy_set_run_bit(int run) {
  ...
  hwrpb_ptr = hwrpb;
  if (hwrpb_ptr && hwrpb_ptr->RSVD_HW[0])
    tree = hwrpb_ptr->RSVD_HW[0];
  else
    return;	/* no tree yet */
#endif
...
void build_fru_table( ) {
    ...
    */
...
#endif
	{
	    gct_base = gct();
	    if (gct_base) {
		hwrpb_ptr->RSVD_HW[0] = gct_base;
		hwrpb_ptr->FRU_OFFSET[0] = gct_base - 0x2000;
		twos_checksum(hwrpb_ptr, offsetof(struct HWRPB,CHKSUM)/4,
		    (UINT *)hwrpb_ptr->CHKSUM);
	    ...
	    addr = (uint64) &(hwrpb_ptr->RSVD_HW[0]);
	    for (i=0; i<1200; i++) {
		ldqp(&addr, &data);
		if (data) {
		    break;
	    ...
	    if (data) {
		hwrpb_ptr->RSVD_HW[0] = data;
		*(UINT *)hwrpb_ptr->FRU_OFFSET = (UINT) (data - virt_to_phys(hwrpb));
		twos_checksum(hwrpb_ptr, offsetof(struct HWRPB,CHKSUM)/4,
		    (UINT *)hwrpb_ptr->CHKSUM);
 ...
 *			to by the SAVED_OWNER pointer in the HWRPB.
 *
...
int gct_init$init_tree(GCT_PLATFORM_DATA *platform_data, 
  ...
  */
  pRoot = (GCT_ROOT_NODE *) _GCT_MAKE_ADDRESS( 0);
#if GCT6
  pRoot->partitions_capable = GALAXY;
  pRoot->partitions_callback = GALAXY_CALLBACKS;
#endif
  ...
  */
  ...
  dsrdb = (int)hwrpb_ptr + *hwrpb_ptr->DSRDB_OFFSET; 
      ...
      */
      if (partition == 0) {
        ...
        */
	    ...
	    status = gct__assign_hw_to_community( community, node, 1);
          atype = 0;
        }
      }
      else {
...
int gct_init$configure_built_galaxy( char *galaxy_id ) {
  ...
  status = gct__assign_hw_to_community( community, smb_node, 1);
  if (status != SUCCESS) {
 ...
 *				      to bind the node (SAVED_OWNER/CONFIG)
...
#else
      pNode->saved_owner = _GCT_MAKE_HANDLE( pNext);
#endif
      found = 1;
      break;
    }
...
int cb_galaxy_pid;
struct SEMAPHORE cb_galaxy_sem;
GCT_HANDLE cb_galaxy_node;
...
void cb_galaxy( struct impure *IMPURE )
	...
	pprintf( "cb_galaxy on CPU %d, code = %d\n", whoami( ), code );
#endif
    ...
    *(uint64 *)&IMPURE->cns$gpr[0][0] = BADPARAM;
	...
	case POWER_HW:
		...
		{
		pprintf( " node = %x\n", node );
		pprintf( " node_change_counter = %x\n", node_change_counter );
		pprintf( " state = %x\n", state );
		pprintf( "calling gct$power_hw\n" );
		}
#endif

#if 0
	    status = gct$power_hw( node, node_change_counter, state );
#else
	    if( cb_galaxy_pid )
		{
		if( !primary( ) )
		...
		else if( node != cb_galaxy_node )
		    {
		    status = NOTDONE;
		    krn$_sleep( 0 );
		    }
		else if( krn$_findpcb( cb_galaxy_pid ) )
		    {
		    spinunlock( &spl_kernel );
		    status = NOTDONE;
		    krn$_sleep( 0 );
		    ...
		    {
		    status = krn$_wait( &cb_galaxy_sem );
		    krn$_semrelease( &cb_galaxy_sem );
		    cb_galaxy_pid = 0;
		    }
		}
	    else
		...
		if( !primary( ) )
		    ...
		    {
		    krn$_seminit( &cb_galaxy_sem, 0, "done" );
		    cb_galaxy_pid = krn$_create( gct$power_hw, null_procedure, &cb_galaxy_sem, 5, -1, 0,
			    "power_hw", "nl", "r", "nl", "w", "nl", "w", node, node_change_counter, state );
		    cb_galaxy_node = node;
		    status = NOTDONE;
		    }
		}
	...
	case SET_ERROR_TARGET:
	    {
	    GCT_HANDLE current_target;
	    GCT_HANDLE new_target;
	    int status;

#if CALLBACK_DEBUG
	    if( callback_debug_flag & 0x2000 )
		pprintf( "SET_ERROR_TARGET\n" );
#endif
	    current_target	= argv[0];
	    new_target		= argv[1];

#if CALLBACK_DEBUG
	    if( callback_debug_flag & 0x4000 )
		{
		pprintf( " current_target = %x\n", current_target );
		pprintf( " new_target = %x\n", new_target );
		pprintf( "calling gct$set_error_target\n" );
		}
#endif

	    status = gct$set_error_target( current_target, new_target );
...
int gct$power_hw( GCT_HANDLE node, int node_change_counter, uint32 state ) {
#if !GCT6
  return (NOTIMPLEMENTED);
#else
  GCT_NODE *pNode;

  _GCT__LOCK_TREE(LOCK_FOR_READ, GCT__INTERNAL_ID, POWER_HW, GCT__RETRY_MAX);

  status = gct__validate_handle(node);

  if (status != SUCCESS) {
    if (tree_debug)     printf("Invalid Node Handle\n");
    gct__unlock_tree(UNLOCK_READ, GCT__INTERNAL_ID);
    return (status);
  }
  ...
  if (!pNode->node_hotswap) {
    gct__unlock_tree(UNLOCK_READ, GCT__INTERNAL_ID);
    return (ILLEGAL);
  }

  if (state == pNode->node_pwr_down) {
    if (pNode->type == NODE_CPU) {
      gct__unlock_tree(UNLOCK_READ, GCT__INTERNAL_ID);
#if WILDFIRE
      if (state)
	status = wf_power_on_cpu(((GCT_NODE_ID_CPU *)&pNode->id)->node_id_cpu_cpu);
      else
	status = wf_power_off_cpu(((GCT_NODE_ID_CPU *)&pNode->id)->node_id_cpu_cpu);
#endif
      pNode->change_counter++;
...
/*
 *	Routine:	gct$set_error_target
 *
 *	Abstract:
 *
 *	  This routine clears the error target in the current node and sets it in the new node.
 *
 *	Inputs:	current_target - Handle of current error target (may be zero)
 *		new_target     - Handle of partition node to become error target
 *
 *	Outputs:SUCCESS - New partition is error target
 *		BADHANDLE - One of the specified handles is invalid
 *		NOTPARTITION - One of the specified nodes is not a partition
 *		CHANGECOLLISION - current partition is not the error target
 *
 */

int32 gct$set_error_target ( GCT_HANDLE current_handle, GCT_HANDLE new_handle ) {
#if !GCT6
  return (NOTIMPLEMENTED);
#else
  int32 status;
  GCT_PARTITION_NODE *current_pNode;
  GCT_PARTITION_NODE *new_pNode;

  _GCT__LOCK_TREE(LOCK_FOR_UPDATE, GCT__INTERNAL_ID, SET_ERROR_TARGET, GCT__RETRY_MAX);

  /* If current error target was specified, validate the node */
  if (current_handle == 0)
	current_pNode = 0;
  else
  {
  	status = gct__validate_handle( current_handle );
  	if (status != SUCCESS) {
      		gct__unlock_tree(UNLOCK_NO_UPDATE, GCT__INTERNAL_ID);
      		return (status);
  	}
	current_pNode = (GCT_PARTITION_NODE *) _GCT_MAKE_ADDRESS(current_handle);
	if (current_pNode->hd_partition.type != NODE_PARTITION) {
      		gct__unlock_tree(UNLOCK_NO_UPDATE, GCT__INTERNAL_ID);
		return (NOTPARTITION);
	}
	if (current_pNode->error_target == 0) {
      		gct__unlock_tree(UNLOCK_NO_UPDATE, GCT__INTERNAL_ID);
		return (CHANGECOLLISION);
	}
  }

  /* Check new error target and get its node pointer */
  status = gct__validate_handle( new_handle );
  if (status != SUCCESS) {
	gct__unlock_tree(UNLOCK_NO_UPDATE, GCT__INTERNAL_ID);
	return (status);
  }
  new_pNode = (GCT_PARTITION_NODE *) _GCT_MAKE_ADDRESS(new_handle);
  if (new_pNode->hd_partition.type != NODE_PARTITION) {
      		gct__unlock_tree(UNLOCK_NO_UPDATE, GCT__INTERNAL_ID);
		return (NOTPARTITION);
  }

  /* Clear error target in current partition node */
  if (current_pNode) {
	current_pNode->error_target = 0;
	current_pNode->hd_partition.change_counter++;
  }

#if WILDFIRE
  /* Program IOPs to send errors to this new target */
  wf_set_error_target(((GCT_NODE_ID_PARTITION *)&new_pNode->hd_partition.id)->node_id_part_part);
#endif

  /* Set error target in new partition node */
  new_pNode->error_target = 1;
  new_pNode->hd_partition.change_counter++;
 ...
 * SIDE EFFECTS:
 *
 *	03-Jan-00  For some unknown reason, this code prevents
 *      the AlphaBIOS from being loaded by the console. This is why
 *      the #if !CORELLE preprocessor statement is in place.
 *
-*/

int gctverify (int argc, char *argv[]) {
  ...
  buf = hwrpb_ptr->FRU_OFFSET[0] + 0x2000;
