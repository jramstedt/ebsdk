...
int build_slot(struct HWRPB *hwrpb,int cpu_no) {
	...
	*(UINT *)slot->PAL_MEM_LEN = 0;
	*(UINT *)slot->PAL_SCR_LEN = 0;
	*(UINT *)slot->PAL_MEM_ADR = 0;
	*(UINT *)slot->PAL_SCR_ADR = 0;
	*(UINT *)slot->PAL_LOGOUT = virt_to_phys(SLOT$_PAL_LOGOUT
		+ cpu_no * SLOT$_PAL_LOGOUT_LEN);
	*(UINT *)slot->PAL_LOGOUT_LEN = SLOT$_PAL_LOGOUT_LEN;
	/*
...
#if WILDFIRE
	*(UINT *)slot->CC_FREQ = krn$_timer_get_cycle_time (cpu_no);
#endif
...
#ifdef PAL$CODE_SIZE
	if( chunk->flags & CHUNK_FLAG_PALCODE )
	    bitmap_size += PAL$CODE_SIZE / HWRPB$_PAGESIZE;
#endif
