 ...
 *      
 *      jsz     20-dec-1999     In show_power check for non-zero number of events instead of just
...
console_restart( )
{
...
#if SHARK
	/* Also fill COM port back to SROM. */
	MB8574AndOr(~MB8574_RESETREQUEST_RW,0);
	MB8574AndOr(~0,MB8574_RESETREQUEST_RW|MB8574_SERIALPORTSELECT_RW);
#else
#if SWORDFISH
	if (SwordFish())
	{
	...
	{
		MB8574AndOr(~MB8574_RESETREQUEST_RW,0);
		MB8574AndOr(~0,MB8574_RESETREQUEST_RW);
	}
#else
    outtig( NULL, 0xE00004, 1 );
#endif
...
*/               
#define PLATFORM_SHARK_DSRDB \
    { 1838, 15, -1, -1, -1, -1, -1, -1, 1050, 1050, "API CS20D %3d MHz" }

#define PLATFORM_UP2000P_DSRDB \
    { 1838, 15, -1, -1, -1, -1, -1, -1, 1050, 1050, "UP2000+ %3d MHz" }
...
#if SHARK
			PLATFORM_SHARK_DSRDB,                     /* 0 */
#elif SWORDFISH
			PLATFORM_UP2000P_DSRDB,                   /* 0 */
#else
			PLATFORM_DP264_DSRDB,                     /* 0 */
#endif
...
#if SWORDFISH
	if (SwordFish())
		sprintf( t, "UP2000 %3d MHz", cpu_speed );
	else
#endif
...
** to first open the temperature sensor device on the server management 
** card. This device in only present on the AlphaServer DS20 and DS20E.
** If successful, attempt to open the DS20E OCP device to determine 
** which type of AlphaServer this is.
**
...
**     34            8          AlphaServer DS20E
**
*/
    if ( fp = fopen( "iic_rcm_temp", "sr+" ) ) {	/* Goldrush AND Goldrack */
    	fclose( fp );
	if ( fp = fopen( "iic_8574_ocp", "sr+" ) ) {
	    sysvar |= 8 << 10;				/* Goldrack DS20E */
	    fclose( fp );
	}
	else
	    sysvar |= 6 << 10;				/* Goldrush DS20 */
    }
    else						/* DP264 */
 	sysvar |= 1 << 10;
#endif
...

get_bcache_info( struct HWRPB *hwrpb, int cpu, uint32 *size )
{
    struct impure *impure_ptr;
    ...
    impure_ptr = ( struct impure * )( PAL$IMPURE_BASE + PAL$IMPURE_COMMON_SIZE + ( cpu * PAL$IMPURE_SPECIFIC_SIZE ) );
