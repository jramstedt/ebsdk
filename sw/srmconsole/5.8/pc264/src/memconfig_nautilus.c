 ...
 * RETURN CODES:
 *
 *	Size of array in Bytes, otherwise, zero (array disabled)
 *
 * ARGUMENTS:
 *
...
{
	IRONGATE_BASEADDRESSCS irongateBaseAddressCS;
	ULONG sizeMB, row;

	for (sizeMB = 0, row = 0; row < NAUTILUS_ROWS_PER_BANK; row++)
	{
		/*
		// Read Configured Size From Irongate.
		*/
		irongateBaseAddressCS.AsUSHORT = NBridgeReadPciConfig(NAUTILUS_PCI_DEVICE_ID,IRONGATE_0_BASEADDRESSCS0+(sizeof(irongateBaseAddressCS)*(array*NAUTILUS_ROWS_PER_BANK+row)),sizeof(irongateBaseAddressCS));

		if (irongateBaseAddressCS.u.bankEnable)
			sizeMB += (irongateBaseAddressCS.u.addressMask+1)*8;	/* Smallest Increment is 8MB. */
	}
...
uint64 get_array_base(int array)
{
	...
	// Read Configured Size From Irongate.
	*/
	irongateBaseAddressCS.AsUSHORT = NBridgeReadPciConfig(NAUTILUS_PCI_DEVICE_ID,IRONGATE_0_BASEADDRESSCS0+(sizeof(irongateBaseAddressCS)*array*NAUTILUS_ROWS_PER_BANK),sizeof(irongateBaseAddressCS));

	if (irongateBaseAddressCS.u.bankEnable)
		base = (ULONG)irongateBaseAddressCS.u.baseAddress << 23;	/* Smallest Increment is 8MB. */
