...
void dump_amd_csrs( void );
void dump_ali_csrs ( void );
...
{  
    ...
    printf("This system utilizes SRM console for OpenVMS, Digital UNIX and Linux\n\n");
...
void dump_amd_csrs( void )
{
	...
	printf("\nAMD-751 PCI(Function 0, Device 0) Configuration Registers:\n");
	for (j = 0; j < 0xff; j = j + 16) {
		printf("R%02x:%08x  ",j,      incfgl(&pb, j));
		printf("R%02x:%08x  ",j + 4,  incfgl(&pb, j + 4 ));
		printf("R%02x:%08x  ",j + 8,  incfgl(&pb, j + 8 ));
		printf("R%02x:%08x\n",j + 12, incfgl(&pb, j + 12));
	}
	printf("\n");
        read_with_prompt("\nEnter to continue",80,buf,0,0,1);
	...
	printf("AMD-751 AGP(Function 0, Device 1) Configuration Registers:\n");
	for (j = 0; j < 0xff; j = j + 16) {
		printf("R%02x:%08x  ",j,      incfgl(&pb, j));
		printf("R%02x:%08x  ",j + 4,  incfgl(&pb, j + 4 ));
		printf("R%02x:%08x  ",j + 8,  incfgl(&pb, j + 8 ));
	...
	pb.slot = 5;
	pb.function = 0;
	if ( incfgl(&pb, 0x00) != 0xffffffff ) {
		buf[0] = 'N';
        	read_with_prompt("\ndump AGP Slot CSRs ? (Y/<N>) ",80,buf,0,0,1);
        	if(toupper(buf[0]) == 'Y' ) {
			dump_agp_csrs();
		}
	}
...
void dump_agp_csrs( void )
{
	...
	printf("\nAGP Slot Configuration Registers:\n");
	printf("Bus %x, Slot %x\n", pb.bus, pb.slot);
	for (j = 0; j < 0xff; j = j + 16) {
		printf("R%02x:%08x  ",j,      incfgl(&pb, j));
		printf("R%02x:%08x  ",j + 4,  incfgl(&pb, j + 4 ));
...
ShowUSB ()
{
	...
	printf ("USB OpenHCI Operational Memory Register\n\n");
	for (i = 0; i < 0x64; i += 4 ) {
		if ((i != 0) && !(i %16)) printf("\n");
		printf("%02x:%08x ", i, inmeml(NULL, bas+i));
	}

	printf ("\n\nLegacy Support Interface Register\n\n");
	printf("%03x:%08x ", 0x100, inmeml(NULL, bas+0x100));
	printf("%03x:%08x ", 0x104, inmeml(NULL, bas+0x104));
	printf("%03x:%08x ", 0x108, inmeml(NULL, bas+0x108));
	printf("%03x:%08x\n", 0x10c, inmeml(NULL, bas+0x10c));
}
