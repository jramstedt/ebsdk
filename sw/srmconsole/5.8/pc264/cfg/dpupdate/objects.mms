! DPUPDATE Target-specific dependencies
i_objects = -
...
objects = $(i_objects), -
	...
	cp$obj:xlate.obj,-
	cp$obj:pc264.obj,-
	cp$obj:pc264_init.obj,-
	cp$obj:tsunami_io.obj,-
	cp$obj:pc264_io.obj,-
	cp$obj:pc264_mcheck.obj,-
	cp$obj:memconfig_pc264.obj,-
	cp$obj:show_config_pc264.obj,-
	cp$obj:amdflash_support.obj,-
	cp$obj:dp264_info.obj,-
	...
	cp$obj:iic_driver.obj,-
	cp$obj:sable_ocp_driver.obj,-
...
cp$obj:grp_base.obj : -
	...
	cp$src:pc264.c, $(pc264_depend),-
	cp$src:pc264_init.c, $(pc264_init_depend),-
	cp$src:tsunami_io.c, $(tsunami_io_depend),-
	cp$src:pc264_io.c, $(pc264_io_depend),-
	cp$src:pc264_mcheck.c, $(pc264_mcheck_depend),-
	cp$src:memconfig_pc264.c, $(memconfig_pc264_depend),-
	cp$src:show_config_pc264.c, $(show_config_pc264_depend),-
	...
	cp$src:lfu_startup_driver.c, $(lfu_startup_driver_depend)
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	/plus/obj=cp$obj:grp_base.obj-
	/lis=cp$lis:grp_base.lis-
	cp$src:alphamm.c+ansi+aputchar+atoi+call_backs+dynamic+entry+ev_action+externdata+filesys+hwrpb+ie+kernel+kernel_support+memset+printf+probe_io+process+powerup+qscan+queue+random+regexp+runbios+scsi+scs+semaphore+shellsort+strfunc+timer-
	+pc264+pc264_init+tsunami_io+pc264_io+pc264_mcheck+memconfig_pc264+show_config_pc264+amdflash_support+decode_alpha+diag_support+diag_evs+secure+symbols+lfu_support_apc+dup+lfu_startup_driver
	! Group "base" command length was ~545

!
! Group "command" (52 files)
!
cp$obj:grp_command.obj : -
	...
	cp$src:dp264_info.c, $(dp264_info_depend),-
	...
	cp$src:galaxy_pc264.c, $(galaxy_pc264_depend),-
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	cp$src:blex.c+bshell+bshell_cmd+boot+builtins+build_fru+exdep+iobq_cmd+isacfg+readline+run_cmd+show_hwrpb+show_map+setshow+startstop+tr+uptime+xlate+dp264_info+dp264_fru+allocfree+bpt+cat+check+chmod+cmp+element+echo+edit+eval+find_field-
	+grep+galaxy+galaxy_pc264+galaxy_config_tree+hd+line+login_cmd+ls+more+prcache+prcache_pci+ps+rm+sem_cmd+sleep+sort+sound+truefalse+validate+wc+lfu
	! Group "command" command length was ~509

!
! Group "driver" (78 files)
!
cp$obj:grp_driver.obj : -
	...
	cp$src:iic_driver.c, $(iic_driver_depend),-
	cp$src:sable_ocp_driver.c, $(sable_ocp_driver_depend),-
	...
	cp$src:smcc669_driver.c, $(smcc669_driver_depend),-
	...
	cp$src:lfu_pc264_driver.c, $(lfu_pc264_driver_depend),-
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	cp$src:hwrpbtt_driver.c+mopdl_driver+moplp_driver+ni_datalink+moprc_driver+moprctt_driver+simport+nl_driver+rd_driver+toy_driver+pci_driver+from_driver+nvram_driver+iic_driver+sable_ocp_driver+tt_driver+dv_driver+vga_bios_driver+kbd_driver-
	+combo_driver+ev_driver+el_driver+pmem_driver+vmem_driver+examine_driver+ev6_ipr_driver+gpr_driver+ps_driver+ndbr_driver+mop_driver+fpr_driver+pi_driver+decode_driver+tee_driver+para_driver+inet_driver+fat_driver+smcc669_driver+dq_driver-
	+de205_driver+n810_driver+pke_driver+pci_nvram_driver+pdq_driver+dac960_driver+pks_driver+isp1020_driver+ew_driver+ei_driver+nport_q+cipca_common+cipca_response+cipca_support+cipca_io+mscp+cipca_driver+kgpsa_driver+wwidmgr+pue_driver-
	+mc_driver+i2o_driver+i2oexec+i2omsg+i2outil+i2o_bsa+i2o_bsa_osm+i2o_scsi_osm+iso9660_driver+aic78xx_driver+aic78xx_him_hwm+aic78xx_him_rsm+aic78xx_him_xlm+bios_disk_driver+lfu_pc264_driver+lfu_kzpsa_driver+lfu_defpa_defea_driver-
	+lfu_cipca_driver+lfu_mc2_driver
	! Warning: Group "driver" command length was ~1102
!
! Group "data" (1 files)
!
cp$obj:grp_data.obj : -
...
slis = -
	...
	cp$lis:xlate.slis,-
	cp$lis:pc264.slis,-
	cp$lis:pc264_init.slis,-
	cp$lis:tsunami_io.slis,-
	cp$lis:pc264_io.slis,-
	cp$lis:pc264_mcheck.slis,-
	cp$lis:pc264_util.slis,-
	cp$lis:memconfig_pc264.slis,-
	cp$lis:show_config_pc264.slis,-
	cp$lis:amdflash_support.slis,-
	cp$lis:dp264_info.slis,-
	...
	cp$lis:iic_driver.slis,-
	cp$lis:sable_ocp_driver.slis,-
...
cp$obj:encap_pwrup.obj : cp$src:pwrup_dp264
	mcr cp$kits:[encapsulate.new]encapsulate cp$src:pwrup_dp264 cp$src:encap_pwrup.mar encap_pwrup -script
...
cp$obj:encap_mfgtest.obj : cp$src:dp264_mfgtest
	mcr cp$kits:[encapsulate.new]encapsulate cp$src:dp264_mfgtest cp$src:encap_mfgtest.mar encap_mfgtest -script
...
cp$obj:pc264.obj : cp$src:pc264.c, $(pc264_depend)
cp$obj:pc264_init.obj : cp$src:pc264_init.c, $(pc264_init_depend)
cp$obj:tsunami_io.obj : cp$src:tsunami_io.c, $(tsunami_io_depend)
cp$obj:pc264_io.obj : cp$src:pc264_io.c, $(pc264_io_depend)
cp$obj:pc264_mcheck.obj : cp$src:pc264_mcheck.c, $(pc264_mcheck_depend)
cp$obj:memconfig_pc264.obj : cp$src:memconfig_pc264.c, $(memconfig_pc264_depend)
cp$obj:show_config_pc264.obj : cp$src:show_config_pc264.c, $(show_config_pc264_depend)
cp$obj:amdflash_support.obj : cp$src:amdflash_support.c, $(amdflash_support_depend)
cp$obj:dp264_info.obj : cp$src:dp264_info.c, $(dp264_info_depend)
...
cp$obj:galaxy_pc264.obj : cp$src:galaxy_pc264.c, $(galaxy_pc264_depend)
...
cp$obj:iic_driver.obj : cp$src:iic_driver.c, $(iic_driver_depend)
cp$obj:sable_ocp_driver.obj : cp$src:sable_ocp_driver.c, $(sable_ocp_driver_depend)
...
cp$obj:smcc669_driver.obj : cp$src:smcc669_driver.c, $(smcc669_driver_depend)
...
cp$obj:lfu_pc264_driver.obj : cp$src:lfu_pc264_driver.c, $(lfu_pc264_driver_depend)
...
cp$obj:lfu_startup_driver.obj : cp$src:lfu_startup_driver.c, $(lfu_startup_driver_depend)
