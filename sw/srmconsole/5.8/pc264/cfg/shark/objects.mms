! SHARK Target-specific dependencies
i_objects = -
...
cp$obj:grp_base.obj : -
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	+pc264+pc264_init+tsunami_io+pc264_io+pc264_mcheck+memconfig_pc264+show_config_pc264+amdflash_support+m1543c_sbridge+m5229_ide+api_debug+showpwr_apitsun+cchip_iic+m1543c_iic+decode_alpha+diag_support+diag_evs+secure+symbols+memtest-
	+memtest_gray_lw+memtest_march+memtest_random+memtest_victim+nettest+lfu_support_apc+dup
	! Group "base" command length was ~682
...
cp$obj:grp_command.obj : -
	...
	cp$src:galaxy_shark.c, $(galaxy_shark_depend),-
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	+edit+eval+exer+find_field+grep+galaxy+galaxy_shark+galaxy_config_tree+hd+help+line+login_cmd+ls+more+net+prcache+prcache_pci+ps+rm+sem_cmd+sleep+sort+sound+truefalse+validate+wc+lfu
	! Group "command" command length was ~545

!
! Group "driver" (76 files)
!
cp$obj:grp_driver.obj : -
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	cp$src:hwrpbtt_driver.c+mopdl_driver+moplp_driver+ni_datalink+moprc_driver+moprctt_driver+simport+nl_driver+rd_driver+toy_driver+pci_driver+from_driver+nvram_driver+api_iic+tt_driver+vga_bios_driver+kbd_driver+combo_driver+ev_driver-
	+el_driver+pmem_driver+vmem_driver+examine_driver+ev6_ipr_driver+gpr_driver+ps_driver+ndbr_driver+mop_driver+fpr_driver+pi_driver+decode_driver+tee_driver+para_driver+inet_driver+fat_driver+m1543c_driver+dq_driver+de205_driver+n810_driver-
	+pke_driver+pci_nvram_driver+pdq_driver+dac960_driver+pks_driver+isp1020_driver+ew_driver+ei_driver+nport_q+cipca_common+cipca_response+cipca_support+cipca_io+mscp+cipca_driver+kgpsa_driver+wwidmgr+pue_driver+mc_driver+i2o_driver+i2oexec-
	+i2omsg+i2outil+i2o_bsa+i2o_bsa_osm+i2o_scsi_osm+iso9660_driver+aic78xx_driver+aic78xx_him_hwm+aic78xx_him_rsm+aic78xx_him_xlm+bios_disk_driver+lfu_pc264_driver+lfu_kzpsa_driver+lfu_defpa_defea_driver+lfu_cipca_driver+lfu_mc2_driver
	! Warning: Group "driver" command length was ~1068
!
! Group "data" (1 files)
!
cp$obj:grp_data.obj : -
...
cp$obj:galaxy_shark.obj : cp$src:galaxy_shark.c, $(galaxy_shark_depend)
