! PC264 Target-specific dependencies
i_objects = -
...
cp$obj:grp_base.obj : -
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	cp$src:alphamm.c+ansi+aputchar+atoi+call_backs+dynamic+entry+ev_action+externdata+filesys+hwrpb+ie+kernel+kernel_support+memset+printf+probe_io+process+powerup+qscan+queue+random+regexp+scsi+scs+semaphore+shellsort+strfunc+timer+pc264-
	+pc264_init+tsunami_io+pc264_io+pc264_mcheck+memconfig_pc264+show_config_pc264+amdflash_support+decode_alpha+diag_support+diag_evs+secure+symbols+memtest+memtest_gray_lw+memtest_march+memtest_random+memtest_victim+nettest+lfu_support_apc+dup
	! Group "base" command length was ~594

!
! Group "command" (55 files)
!
cp$obj:grp_command.obj : -
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	cp$src:blex.c+bshell+bshell_cmd+boot+builtins+build_fru+exdep+iobq_cmd+isacfg+isp1020_edit+readline+show_hwrpb+show_map+setshow+startstop+tr+uptime+xlate+dp264_info+dp264_fru+allocfree+bpt+cat+check+chmod+cmp+element+echo+edit+eval+exer-
	+find_field+grep+galaxy+galaxy_pc264+galaxy_config_tree+hd+help+line+login_cmd+ls+more+net+prcache+prcache_pci+ps+rm+sem_cmd+sleep+sort+sound+truefalse+validate+wc+lfu
	! Group "command" command length was ~528
...
cp$obj:grp_driver.obj : -
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	+mc_driver+i2o_driver+i2oexec+i2omsg+i2outil+i2o_bsa+i2o_bsa_osm+i2o_scsi_osm+iso9660_driver+aic78xx_driver+aic78xx_him_hwm+aic78xx_him_rsm+aic78xx_him_xlm+lfu_pc264_driver+lfu_kzpsa_driver+lfu_defpa_defea_driver+lfu_cipca_driver-
	+lfu_mc2_driver
	! Warning: Group "driver" command length was ~1085
!
! Group "data" (1 files)
!
cp$obj:grp_data.obj : -
