! NAUTILUS Target-specific dependencies
i_objects = -
...
cp$obj:grp_base.obj : -
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	+nautilus+nautilus_init+irongate_io+nautilus_io+nautilus_mcheck+memconfig_nautilus+show_config_nautilus+amdflash_support+m1543c_sbridge+m1543c_iic+m5229_ide+api_debug+decode_alpha+diag_support+diag_evs+secure+symbols+memtest+memtest_gray_lw-
	+memtest_march+memtest_random+memtest_victim+nettest+lfu_support_apc+dup
	! Group "base" command length was ~675

!
! Group "command" (58 files)
!
cp$obj:grp_command.obj : -
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	cp$src:blex.c+bshell+bshell_cmd+boot+builtins+build_fru+date_cmd+exdep+iobq_cmd+isacfg+isp1020_edit+readline+run_cmd+show_hwrpb+show_map+setshow+startstop+tr+uptime+xlate+nautilus_nbridge+nautilus_info+dp264_fru+allocfree+bpt+cat+check+chmod-
	+cmp+element+echo+edit+eval+exer+find_field+grep+galaxy+galaxy_nautilus+galaxy_config_tree+hd+help+line+login_cmd+ls+more+net+prcache+prcache_pci+ps+rm+sem_cmd+sleep+sort+sound+truefalse+validate+wc+lfu
	! Group "command" command length was ~568
