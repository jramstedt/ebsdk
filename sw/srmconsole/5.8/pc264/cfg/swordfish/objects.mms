! SWORDFISH Target-specific dependencies
i_objects = -
...
objects = $(i_objects), -
	...
	cp$obj:showpwr_apitsun.obj,-
	cp$obj:api_debug.obj,-
	cp$obj:cchip_iic.obj,-
	cp$obj:pcf8584_iic.obj,-
...
cp$obj:grp_base.obj : -
	...
	cp$src:showpwr_apitsun.c, $(showpwr_apitsun_depend),-
	cp$src:api_debug.c, $(api_debug_depend),-
	cp$src:cchip_iic.c, $(cchip_iic_depend),-
	cp$src:pcf8584_iic.c, $(pcf8584_iic_depend),-
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	+pc264+pc264_init+tsunami_io+pc264_io+pc264_mcheck+memconfig_pc264+show_config_pc264+amdflash_support+showpwr_apitsun+api_debug+cchip_iic+pcf8584_iic+decode_alpha+diag_support+diag_evs+secure+symbols+memtest+memtest_gray_lw+memtest_march-
	+memtest_random+memtest_victim+nettest+lfu_support_apc+dup
	! Group "base" command length was ~658

!
! Group "command" (57 files)
!
cp$obj:grp_command.obj : -
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	cp$src:blex.c+bshell+bshell_cmd+boot+builtins+build_fru+date_cmd+exdep+iobq_cmd+isacfg+isp1020_edit+readline+run_cmd+show_hwrpb+show_map+setshow+startstop+tr+uptime+xlate+dp264_info+dp264_fru+allocfree+bpt+cat+check+chmod+cmp+element+echo-
	+edit+eval+exer+find_field+grep+galaxy+galaxy_pc264+galaxy_config_tree+hd+help+line+login_cmd+ls+more+net+prcache+prcache_pci+ps+rm+sem_cmd+sleep+sort+sound+truefalse+validate+wc+lfu
	! Group "command" command length was ~545
...
cp$obj:grp_driver.obj : -
 ...
 $(cc) $(cdefs) $(cquals) $(copt) $(list) -
	...
	cp$src:hwrpbtt_driver.c+mopdl_driver+moplp_driver+ni_datalink+moprc_driver+moprctt_driver+simport+nl_driver+rd_driver+toy_driver+pci_driver+from_driver+nvram_driver+api_iic+iic_driver+tt_driver+dv_driver+vga_bios_driver+kbd_driver-
	...
	! Warning: Group "driver" command length was ~1093
!
! Group "data" (1 files)
!
cp$obj:grp_data.obj : -
...
slis = -
	...
	cp$lis:showpwr_apitsun.slis,-
	cp$lis:api_debug.slis,-
	cp$lis:cchip_iic.slis,-
	cp$lis:pcf8584_iic.slis,-
...
cp$obj:showpwr_apitsun.obj : cp$src:showpwr_apitsun.c, $(showpwr_apitsun_depend)
cp$obj:api_debug.obj : cp$src:api_debug.c, $(api_debug_depend)
cp$obj:cchip_iic.obj : cp$src:cchip_iic.c, $(cchip_iic_depend)
cp$obj:pcf8584_iic.obj : cp$src:pcf8584_iic.c, $(pcf8584_iic_depend)
