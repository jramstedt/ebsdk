...
/*
 ** Build a hardware node for pci and agp buses .
 */
	if ((status = build_bus_hw(iop_id, hose_id, 0)) != SUCCESS)
		return(status);  

	if ((status = build_bus_hw(iop_id, hose_id, NBridgeReadPciConfig(GOBY_PCIPCI_DEVICE_ID,OFFSETOF(GOBY_PCIPCI_CONFIG,SubordinateBusNumber),sizeof(unsigned char)))) != SUCCESS)
		return(status);  
	...
	*/
    for (i = 0;  i < 31; i++)
	{
		for (j = 0;  j <= 31;  j++)
		{
			for (k = 0;  k <= 7;  k++)
			{
	    		if ((pb = get_matching_pb(0, i, j, k, 0)) != (struct pb *)0)
				{
					if (pb->type == TYPE_PCI)
					{
						/*
