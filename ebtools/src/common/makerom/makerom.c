...
#ifndef lint
static char *RCSid = 
  "$Id: makerom.c,v 1.1.1.1 1998/12/29 21:36:23 paradis Exp $";
#endif
 ...
 * Initial CVS checkin
 *
 * Revision 1.23  1996/05/22  15:49:28  fdh
 * Added an option for creating headers with an experimental signature.
 *
 * Revision 1.22  1995/11/30  02:23:46  fdh
 * Modified the header specification to include the HEADER REV EXT
 * field.
 *
 * Revision 1.21  1995/11/27  19:49:38  cruz
 * Updated makerom version to 2.2
 *
 * Revision 1.20  1995/11/14  19:15:31  cruz
 * Cleaned up code with lint.
 *
 * Revision 1.19  1995/11/13  18:47:09  cruz
 * Added code for filling in the optional field that specifies the
 * offset into the ROM where the image should be written.
 *
...
typedef struct fileinfo {
    ...
    int32 rom_offset;                   /* rom destination offset */
    int rom_offset_flag;                      
...
int main(int argc, char **argv)
{
    ...
    SpecialHeaderSpecified = FALSE;

    for (idx = 0; idx < SEEN_SIZE; idx++)
	seen[idx] = FALSE;
/*
 ...
 */
		    ...
		    case 'r': 
/*
 *  This is a special one, the -r option is followed (immediately)
 *  by the offset where the image should be placed in the rom.
 */
			_SEEN(tolower(option)) = TRUE;
			{
			  char strbuf[16+1];
			  sscanf(arg, "%8s", strbuf);
			  now->rom_offset = strtoul(strbuf, NULL, 16);
                          now->rom_offset_flag = 1;
			}
			arg = arg + strlen(arg);
			break;
 ...
 */
		    ...
		    case 'e': 
/*
 *  This option allows the user to specify a version of
 *  the Makerom header that is not fully supported.
 *  This special version of the header replaces the
 *  first longword of the signature with an Alpha branch
 *  instruction.  This branch allows some images which
 *  begin with the header to be executable because the
 *  branch instruction will branch around the header.
 *  However, the problem is that SROM and Debug Monitor
 *  versions released prior to May 7, 1996 will not
 *  recognize this header.  Further problems exist with
 *  the NT Firmware update utility FWUPDATE.EXE and the
 *  SRM console.  These utilities might never recognize
 *  this header.
 */
			_SEEN(tolower(option)) = TRUE;
			SpecialHeaderSpecified = TRUE;
			arg = arg + strlen(arg);
			break;
 ...
 */
    ...
    if (SpecialHeaderSpecified)
      head.romh.V0.signature = ROM_H_BRANCH_SIGNATURE
	| (((head.romh.V0.hsize-4)/4) & ROM_H_BRANCH_OFFSET_MASK);
    else
...
/*
 *  The header specification was modified to include a HEADER REV EXT
 ...
 *  compatibility.
 */
    head.romh.V1.hversion = (char) 1;
    head.romh.V1.hversion_ext = (char) ROM_H_REVISION-1;
    ...
    head.romh.V2.rom_offset = (ifile->rom_offset & ~3) | (ifile->rom_offset_flag & 1);
...
void usage()
{
    ...
    printf("\t-r,R<offset> = optional offset into the rom where image should go\n");
